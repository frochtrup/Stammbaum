...
**sw-Version:** v495 · Cache: `stammbaum-v495`
...
| STORY | **Story Mode** | Reichhaltige Personen-Erzählung: automatisch generierter Fließtext („Johann Müller wurde 1823 in Köln geboren …") aus GEDCOM-Events; eingebettete Fotos an relevanten Lebenspunkten; Mini-Karte mit Lebensorten; Zeitstrahl (→ F9); Darstellung in `ui-story.js` als scrollbare HTML-Seite; Export als PDF (via `window.print()` + Print-CSS) oder HTML-Download; setzt F9 (Timeline) und MAP-MIGR (Karte) als optionale Komponenten voraus | XL |
