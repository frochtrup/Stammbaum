...
| TREE-SPOUSE-FOCUS | **Fokus-Ehepartner bei Mehrfachehen** | Im Sanduhr- und Nachkommen-Baum wird aktuell die erste vollständige Familie (husb+wife) als Hauptehe gewählt; der ⚭-Button navigiert zur Familiensicht (bleibt so); Fokus-Definition über Personen-Detail: Button „Als Hauptehe im Baum" neben jedem Familieneintrag → schreibt `p._focusFam = famId`, gespeichert in IDB; Baum liest `_focusFam` als Priorität | M |
