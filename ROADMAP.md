...
**sw-Version:** v1057 · Cache: `stammbaum-v1057` · `test-unit.js` = **884 Tests** grün · `test-csp.js` grün · `test-snapshot-place.js` grün · GEDCOM Roundtrip `net_delta=0` stabil · GRAMPS stabil · **Pre-Commit-Gate aktiv** (test-csp + test-unit + test-snapshot-place)
...
| Version(en) | Feature |
|---|---|
| v1043–v1055 | **Farm-PO-Invariante + Roundtrip-Stabilität:** `ev.placeId` darf nie auf Farm/Building-PO zeigen (ADR-027/028-Invariante). `_ensureHofFarmPO`-Zuweisung entfernt; `_placeLink` Fall 1+2b mit Farm/Building-Guard; `_migrateFarmPOsToHofObjects._repoint` via addr-Norm statt placeId-Lookup. Fixes: Lade-Toast nur bei neuen Höfen (nicht Runtime-Re-Links); `markChanged()` nur bei GEDCOM-Delta; Orts-Toast einmal pro File; PLAC mit führendem Leer-Segment kein Pseudo-Hof; Spurious-PO „Oster 82a" nach v1050-Regression; IDB-Reset-UI; Statistik-Kachel Orte. |
