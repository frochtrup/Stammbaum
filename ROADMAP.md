...
**sw-Version:** v472 · Cache: `stammbaum-v472`
...
| GRAMPS-Orte | **Orts-Picker im GRAMPS-Modus** | 3 Schritte: (1) **placeId-Erhalt im Edit-Pfad** — `showEventForm`/`saveEvent` liest `ev.placeId` über hidden field durch; gleiches für birth/death/chr/buri + Familien-Events; verhindert stillen Datenverlust beim ersten Bearbeiten (S). (2) **Place-Picker aus placeObjects** — bestehende Autocomplete-Infrastruktur nutzt `db.placeObjects` als Quelle; Auswahl setzt title + placeId; Freitext ohne Match → Writer legt neues placeObject an wie bisher (M). (3) **Hierarchie-Anzeige im Event-Detail** — `placeId`-Kette über parentId auflösen → `Stadt → Kreis → Land` in Personen-/Familien-Detail; nur GRAMPS-Modus (S) | S+M+S |
