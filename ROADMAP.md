...
**sw-Version:** v998 · Cache: `stammbaum-v998` · `test-unit.js` = **558 Tests** grün · `test-csp.js` grün · `test-snapshot-place.js` grün (2 Goldfiles) · GEDCOM Roundtrip `net_delta=0` stabil · GRAMPS stabil · **Pre-Commit-Gate aktiv** (test-csp + test-unit + test-snapshot-place)
...
- **v998 — Zwei Geo-Validierungsregeln für Höfe.** `validatePlaces()` (Orte-Tab ⚠-Panel + Badge) prüft jetzt zusätzlich: **`HOF_NO_COORD`** — Hof/Gebäude (Typ Farm/Building) ohne Koordinaten (auf der Karte nicht sichtbar); **`HOF_FAR`** — Hof-Koordinaten > `_HOF_MAX_DIST_KM` (25 km, `_placeDistKm`-Haversine) vom umschließenden Ort entfernt (fängt vertauschte lat/long, Zahlendreher, falschen Ort). Anzeige-Labels 🏡 in `CODE_LBL`. Browser-verifiziert an demo.ged (Hof ohne Koord erkannt; Hof 502 km vom Ort markiert).
- **v997 — ADR-026 Phase 2C/2c: Hof-Notiz single-source — Hartschnitt funktional komplett.** Der `[Hof]`-Notiz-Writer wird jetzt aus der **Farm-PO-Notiz** gespeist (`_hofNoteFor(addr)`: Farm-PO bevorzugt, Sidecar-Fallback) statt aus `db.hofObjects`; `_hofNoteText` (addr→Notiz) baut die geteilten `@N_HOF@`-Records, Per-Event-Referenz gated nur noch über `_hofNoteIds`. Damit schreibt die Hof-Notiz-Speicherung **nur noch ins Farm-PO** (kein Sidecar-Write mehr). **2c-Gate-Test (af.16):** Notiz aus Farm-PO bei **leerem Sidecar** → `[Hof]`-Record → überlebt write→parse→derive. Browser-verifiziert: Notiz-Edit ohne Sidecar, `[Hof]` im GEDCOM, Reload-Überleben. **Ergebnis:** Koords **und** Notizen sind jetzt single-source im Farm-placeObject (geräteübergreifend); der `hofObjects`-Sidecar ist **write-frozen** (Migration liest ihn nur noch für unmigrierte Altbestände). +2 Tests (556→558). GEDCOM `net_delta=0` + GRAMPS stable. *Optionaler Rest:* `stammbaum_hofobjects`-localStorage-Key löschen — erst sinnvoll, wenn alle Geräte-Altbestände durch die Migration in Farm-POs geflossen sind.
- **v996 — ADR-026 Phase 2C/2d: Hof-Koords single-source (Sidecar-Write-Stopp) + placeId-Writer.** `saveHofCoord` schreibt nur noch ins Farm-PO (kein Sidecar-Dual-write mehr); `geoLines` (Writer) löst Koordinaten jetzt **über `ev.placeId` → placeObject** auf — aber nur, wenn sie von `obj.lati` abweichen (editierte Koord), sonst byte-genauer `_latiStr`-Roundtrip → `net_delta=0` erhalten. `saveHofRename` überträgt die Farm-PO-Koords/Notiz auf die neue Adresse + entfernt das verwaiste alte Farm-PO. **Browser-verifiziert:** editierte Koord `N50.5` korrekt im GEDCOM (nicht mehr stale `48.137`), überlebt Reload ohne Sidecar, Rename trägt Koord mit. **Notiz bleibt bewusst dual-write** (GEDCOM-Notiz-Export läuft noch über den `[Hof]`-Record, der `db.hofObjects` liest → wird in 2c umgestellt). Gate + GEDCOM/GRAMPS-Roundtrip stabil.
- **v995 — placeObjects-Reload-Persistenz (T0-STORAGE-Fix, entriegelt ADR-026-Hartschnitt).** `_deriveHofAndMigrate` lädt jetzt `loadPlaceObjectsFromIDB` (IDB + OneDrive), bevor die Hof-Migration läuft — auf **allen** Reload-/Restore-Pfaden (storage.js). Behebt einen **vorbestehenden Bug**: placeObjects (Koords/Notizen/pnames/enclosedBy) wurden beim App-Reload gar nicht aus IDB geladen, nur bei explizitem Datei-Öffnen → lokal eingegebene Anreicherungen waren nur über Sidecar/extraPlaces/Events rekonstruierbar. Merge ist additiv-per-id (überschreibt nichts), die Hof-Migration findet geladene Farm-POs via `_findFarmPO` wieder (keine Dubletten). **Browser-verifiziert:** eine nur im Farm-PO/IDB liegende Koord (nicht in Events/Sidecar) überlebt einen simulierten Reload. **Entriegelt** damit die ADR-026-Schritte 2c/2d (Sidecar/`[Hof]` verlustfrei entfernbar).
- **v994 — ADR-026 Phase 2C (Teil 2b): Hof-Schreibwege + Detail auf Farm-PO (dual-write).** `_ensureHofFarmPO(db,addr)` (find-or-create Farm-PO + Dorf-Auflösung + Event-Verknüpfung, aus der Migration extrahiert → geteilte Logik) + `upsertHofPO(addr,patch)` als UI-Schreibweg. `saveHofCoord`/`deleteHofCoord`/Hof-Notiz schreiben jetzt ins **Farm-placeObject** (geräteübergreifend via `savePlaceObjects`), Hof-Detail (Koord-/Notiz-Sektion) liest via `hofMeta`. **Dual-write Sidecar** bewusst beibehalten: `loadPlaceObjectsFromIDB` läuft nur bei Datei-Laden, **nicht beim App-Reload** → der Sidecar (localStorage) ist aktuell die Reload-Überlebens-Mechanik. Reine Sidecar-Entfernung (2d) ist dadurch blockiert (vorbestehende T0-STORAGE-Lücke: placeObjects laden nicht beim Reload). `updateChangedIndicator` null-sicher gemacht. Browser-verifiziert: Notiz/Koord-Edit landet in Farm-PO **und** Sidecar, Detail zeigt Farm-PO-Wert. +8 Tests (af.15 + hofMeta, 548→556).
- **v993 — ADR-026 Phase 2C (Teil 2a): Hof-Koords/Notiz-Lesen aus Farm-PO (Karte + Listen-Badges).** `buildHofIndex` führt die Farm-`placeId` aus den Events mit; neuer Lese-Helfer `hofMeta(hof)` liest Koordinaten/Notiz **primär aus dem Farm-placeObject** (geräteübergreifend via placeObjects-Sync), `hofObjects`-Sidecar nur als Fallback. Karten-Hof-Marker iterieren jetzt `buildHofIndex` + `hofMeta` (statt `hofObjects`), Höfe-Listen-Badges (📍/📝) ebenso. **Browser-verifiziert:** mit geleertem Sidecar behalten alle Demo-Höfe ihre Koords (aus den Farm-POs) → Marker erscheinen auch auf einem Zweitgerät ohne localStorage. Rein additiv (Fallback), **kein Schreibpfad geändert, nichts entfernt** — Detail-Ansicht + Schreibwege folgen in Teil 2b. +7 Tests (af.13/af.14, 541→548). Gate + Roundtrip stabil.
- **v992 — ADR-026 Phase 2C (Teil 1): scoped Hof-Picker + Ort/Hof-Eingabetrennung (browser-verifiziert).** Event-Formular: 🏡-Button am Adressfeld öffnet einen **Hof-Picker** (`openHofPicker`), der Farm/Building-placeObjects mit Dorf-Kontext listet — auf den gewählten Ort eingegrenzt (`enclosedBy`). Auswahl füllt `ev.addr`; Migration verknüpft beim Laden. Umgekehrt zeigt der **Ort-Picker keine Höfe** mehr (Farm/Building gefiltert) → saubere Trennung beider Eingaben. Zusätzlich **`ev.place`-Reprojektion** in der Migration (ADR-024-Invariante): nach dem Umhängen auf die Farm wird `ev.place` auf die placeId-Projektion angeglichen, sonst mischte `collectPlaces` den alten Dorf-String mit dem Farm-Typ (Dorf-Name mit 🏡-Badge). Live verifiziert an `demo.ged`: Hof-Picker scoped (München zeigt nur Sonnenstr./Leopoldstr.), Ort-Picker hof-frei, `ev.place` = volle Farm-Hierarchie, idempotent, 0 Fehler. +1 Test (af.12-Reprojektion, 540→541). **Offen Teil 2:** Writer-Notiz unter PLAC (`[Hof]`-Abbau), `buildHofIndex`/Karte auf placeId, `hofObjects`-Sidecar entfernen, HANDBUCH.
- **v991 — ADR-026 Phase 2B: alle Lade-Pfade + Enclosure-Kette + Typ-Fix (browser-verifiziert).** Drei Korrekturen aus der Live-Verifikation an `demo.ged`: (1) **Alle 5 Lade-Pfade** — Migration lief nur bei explizitem Datei-Öffnen; `loadDemo` + IDB-/localStorage-Restore (jeder Reload!) leiteten hofObjects gar nicht ab (Pre-existing-Bug: Hof-Koords fehlten nach Reload). Zentralisiert in `_deriveHofAndMigrate()`, in alle GEDCOM-Pfade verdrahtet. (2) **Enclosure-Kette statt Komma-Titel** — ein mehrstufiger `ev.place` („München, Bayern, Deutschland") wurde als *ein* PO mit Komma-Titel angelegt; `_buildFormString._atomic` nahm nur „München" → „Bayern, Deutschland" ging verloren. `_ensureVillageChain` baut jetzt echte Glieder München←Bayern←Deutschland → PLAC bleibt vollständig. (3) **Typ-Fix** — promovierte Orte sind `Unknown` statt fälschlich `City` (Stadt/Dorf/Kreis aus nacktem String nicht ableitbar). **Live verifiziert:** `2 PLAC Sonnenstraße 12, München, Bayern, Deutschland`, Binnenmigration innerorts (Sonnenstr./Leopoldstr. = beide München, distinkte Koords), idempotent, 0 Konsolenfehler. +6 Tests (af.11 + Typ, 528→534).
- **v990 — ADR-026 Phase 2 (Teil A): Hof→Farm-Migration verdrahtet + Dorf-Promotion.** Migration läuft jetzt beim Laden (3 Storage-Stellen, **nach** hofObjects-Ableitung — *nicht* in `setDb`, das läuft davor). **Empirischer Befund** (MeineDaten: 12 Höfe): GEDCOM hat keine Dorf-placeObjects → naive Migration verlöre den Ort im PLAC. Korrektur: `_hofVillageString` promoviert das Dorf aus `ev.place` (find-or-create), `_findVillagePO` dedupliziert; führendes Hof-Blatt wird aus `ev.place` gestrippt → **idempotent über den GEDCOM-Roundtrip** (`out1===out2` auf realer Datei verifiziert, 4 Dörfer + 12 Farms, zeilenneutral). Kein Dorf wird angelegt, wenn `ev.place == Adresse` (Schutz gegen Namens-Dopplung). Migration ist `markChanged`-frei → kein Dirty-Status nach Load. +7 Tests (af.8–af.10, 521→528). **Noch offen (Teil B/C):** Writer-Notiz unter PLAC (`[Hof]`-Abbau), Zwei-Felder-Eingabe, Hof-Sicht/Karte auf placeId, hofObjects-Sidecar entfernen.
- **v989 — ADR-026 Phase 1: Hof→Farm-Migration (Funktion + Tests):** `_migrateHofObjectsToPlaceObjects(db)` (gedcom.js) wandelt `hofObjects[addr]` mit Koords/Notiz in erstklassige `placeObjects` vom Typ `Farm` um — `enclosedBy` = häufigstes Dorf der RESI/PROP-Events (`_hofVillageId`), eigene `lat/long/note`; betroffene Events werden auf den Farm-Ort umgehängt (`ev.placeId`), `ev.addr` bleibt als Postdetail. Pure + idempotent, Dedup gegen bestehende Farm-Objekte nach (Name + Dorf-Scope). **Noch nicht in `setDb` verdrahtet** (Verdrahtung + Writer-Anpassung + Entfernen des hofObjects-Sidecars = Phase 2, damit der GEDCOM-PLAC-Übergang `Ochtrup → "Hof…, Ochtrup"` koordiniert + idempotent-stabil bleibt). +24 Unit-Tests (Gruppe (af), 497→521). Ziel: geräteübergreifende Hof-Koordinaten + sichtbare Binnenmigration. Konzept: ARCHITECTURE.md ADR-026.
- **v987–v988 — Hof-Notiz vs. RESI-Event-Notiz getrennt:** Eine getippte RESI/PROP-Event-Notiz wurde beim Reload zur Hof-Notiz (`_derivedHofObjectsFromDb` leitete `hof.note` aus *jeder* Event-Notiz ab) — keine Trennung. **v987** behob zunächst eine Über-Unterdrückung (Event-Notiz verschwand bei Streu-Hof unter abweichendem Adress-Key, weil globales `_allHofNoteTexts` sie verschluckte). **v988** trennt beide sauber per **GEDCOM-konformem Text-Präfix `[Hof] `** in einer normalen `NOTE` (kein Custom-Tag, in jedem Programm lesbar): Parser leitet `hof.note` nur aus `[Hof]`-präfixierten Notizen ab; Writer schreibt Hof-Notizen als `[Hof]`-präfixierten `@N_HOF@`-Record, event-eigene Notizen bleiben präfixlos. Anzeige/Buch schließen präfixierte Teile aus der Event-Notiz aus; Hof-Editor speichert Klartext. Round-trip stabil, beide Notiz-Arten koexistieren.
...
**Handbuch-Stand: sw v998 *(aktuell)*** — beide Versionsfelder im HANDBUCH (Header + Footer) auf v998. Neu (Kap. 7): zwei Geo-Validierungsregeln „🏡 Hof ohne Koordinaten" + „🏡 Hof zu weit vom Ort" im Plausibilitäts-Validator. Neu (Kap. 15): eigene geräteübergreifende Hof-Koordinaten + Binnenwanderung auf der Karte (ADR-026), 🏡-Hof-Picker bei der Wohnort-Eingabe + Ort/Hof-Trennung. *Bewusst ohne Handbuch-Eintrag (rein intern):* Farm-placeObject-Migration, hofObjects-Sidecar/Dual-write→single-source, Enclosure-Ketten, Reprojektion, placeObjects-Reload-Persistenz, `[Hof]`-Writer-from-Farm-PO — die korrekte Anzeige/Speicherung ist für Nutzer transparent. Zuvor: Hof-Notiz vs. Wohnort-Notiz-Trennung mit `[Hof]`-Marker (Kap. 15). Neu dokumentiert: Boot-Loader/Start-Anzeige (Kap. 2), verwaiste Quellbezüge (⚠-Badge + ✕-Entfernen, Kap. 6) + Validator-Regel „Quellbezug ohne vorhandene Quelle" (Kap. 13, jetzt 31 Regeln), Quellen-Auswahl bei Aufgaben (Kap. 13), Orts-Dubletten-Badge auf ⇉ (Kap. 7). Bereits zuvor dokumentiert: 12 Druckausgaben (Kap. 20), Ort-Steckbrief inkl. Verwaltungs-Zeitlinie WYSIWYG (Kap. 7), Template-Generator (Kap. 4), periodengerechte Ortsanzeige mit 🏘-Sprung (Kap. 4), RESI-Datums-Schnellhilfe + Hof-Notiz-Automatik (Kap. 15), Skalierung/Barrierefreiheit in FAQ. *Bewusst ohne Handbuch-Eintrag (rein interne Änderungen):* View-Robustheit-Internals P0–P6, Koord-Single-Source, Orts-Projektions-Invariante (v983–v985, intern — die korrekte historische Ortsanzeige ist für Nutzer transparent), T0-UI-Tests, CSP-Durchsetzung + Pre-Commit-Gate, T0-FUNC-SPLIT, Vorname-Normalisierung. *(Offen: echte Screenshots statt Mockups → DOC-SCREENS.)*
