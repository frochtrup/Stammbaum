...
**sw-Version:** v773 · Cache: `stammbaum-v773`
...
| **2** | **RES-EVAL** | **Quellenbewertung (Evidenzmodell)** — 3 Achsen je Zitat: Quellentyp (Original/Abschrift/Autorenwerk) · Information (primär/sekundär) · Evidenz (direkt/indirekt/negativ); „Informant" via ASSO-Rolle. **+ Repository-Rest:** Archivtyp, mehrere Signaturen, Findbuch-URL. Speist Dashboard (Schwach-Quellen-Flag). | `citation.eval` als `_EVAL`-Tag (ADR-022) | **2a ✅ sw v773** · offen: 2b–2e |

**RES-EVAL Teilschritte:** **2a Kern (GEDCOM) ✅ sw v773** — `citation.eval`+`EVAL_AXES`/`_newEval`/`evalIsEmpty`/`_evalToQuay` (gedcom.js), Parser-Extraktion `_EVAL` (modelliert, kein Doppel-Schreiben — ADR-022), Writer-Hook in `_writeSourCits` (deckt alle Zitat-Kontexte), GED7-SCHMA-Deklaration, Strict-Strip. Verifiziert: `net_delta=0`+`out1===out2` auf `_EVAL`-Fixture, +18 Unit-Tests (123 total). **Offen: 2b UI** (Bewertungs-Aufklapper pro Zitat-Tag in `renderSrcTags` + `updateSrcEval` + „QUAY aus Bewertung"), **2c Validator/Dashboard** (`MISSING_EVAL`-Regel + Lückenradar-Balken), **2d GRAMPS** (Citation-`<attribute>`-Mapping, Roundtrip-Test), **2e Repository-Rest** (`rtype`-Select, Findbuch-URL).
...
**Handbuch-Stand: sw v769** *(veraltet — v770–v773 [QUICK-TPL age-Feldtyp, Qualitäts-Dashboard RES-DASH, Evidenzmodell RES-EVAL 2a] noch nicht dokumentiert. v750–v758 [Test-Automation, GRAMPS-/Parser-Fixes, ESM-Migration, PAGE-URL-Migration] sind Infrastruktur ohne nutzersichtbare Bedienung und bewusst nicht im Endnutzer-Handbuch.)*
