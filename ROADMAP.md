...
| STORY | **Story Mode** | Reichhaltige Personen-Erzählung aus GEDCOM-Events: Markdown-Template mit Platzhaltern (Name, Daten, Orte) → clientseitig befülltes HTML → PDF via `window.print()` + Print-CSS; Fließtext-Gerüst automatisch generiert („Johann Müller wurde 1823 in Köln geboren …"); eingebettete Fotos (base64) an Lebenspunkten; Mini-Karte mit Lebensorten (→ MAP-MIGR); Zeitstrahl (→ F9); `ui-story.js`; Download als HTML oder PDF | XL |
