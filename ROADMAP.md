...
**sw-Version:** v478 · Cache: `stammbaum-v478`
...
| TREE-SPOUSE-FOCUS | **Fokus-Ehepartner bei Mehrfachehen** | Bevorzugte Familie durch Reihenfolge der `FAMS`-Einträge definiert (Index 0 = bevorzugt) — analog zu `_PRIM` bei Medien. Baum liest bereits `p.fams[0]` als Hauptehe (kein neuer Code nötig). UI im Personen-Detail: Pfeil-Button oder „Als Hauptehe" neben jedem Familieneintrag verschiebt Eintrag in `p.fams[]` auf Position 0, setzt `AppState.changed`. GEDCOM-Roundtrip verlustfrei (Reihenfolge der `1 FAMS`-Zeilen). Kein IDB-Sonderschlüssel, kein neuer GEDCOM-Tag. | S |
