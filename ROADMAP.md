...
|---|---|---|
| v899 | **SCALE-TEST + SORT-CACHE** — Synthetisches 20k-GEDCOM (`generate-scale-test.js`, 6,2 MB, 446k Zeilen, Roundtrip net_delta=0 ✅). Messung via `test-scale.js` identifiziert Sort als dominanten Engpass: `renderPersonList` rief `localeCompare`-Sort bei jedem Tab-Wechsel neu auf (~940 ms JXA / ~320 ms Chromium bei 20k). Fix: `UIState._personSortCache = { mode, count, sorted }` — Cache-Hit wenn ungefilterte Vollmenge (`persons.length === allCount`) + gleicher Sort-Modus. Invalidierung: `markChanged()` → `_invalidatePersonListCache()` (ui-views-person.js), `_finishLoad` GEDCOM + GRAMPS (storage-file.js). Parser-Worker bereits vorhanden (`gedcom-worker.js` + `storage-file.js:_processLoadedText`). Details: `SCALE-TEST-BEFUNDE.md`. | Tab-Wechsel zur Personenliste nach erstem Render: 0 ms Sort-Overhead statt ~320–940 ms. Skaliert sauber auf 20k+ Personen. |
| v892 | **Ortsreport + Ortsbuch-Export** — `showPlaceDetail()` erweitert um 3 neue Sektionen: (1) **Häufigste Familiennamen** (Top-10 Balkendiagramm + Top-5 Vornamen als Chips, dedupliziert nach Person-ID); (2) **Zeitraumverteilung** (adaptive Buckets: 25/50/100 Jahre je nach Ereignisdichte und Zeitspanne, Mini-Balken + ausklappbare Detaillisten); (3) **Hierarchie-Timeline** (vollständige `enclosureChainAsOf()`-Kette zu Schlüsseljahren aus `enclosedBy[]`). `exportOrtsbuch()` in `ui-print.js` erzeugt standalone-HTML mit Inhaltsverzeichnis, pro-Ort-Sektionen (OSM-Tile-Karte, Hierarchie, historische Namen, Nachnamen-Chips, Ereignis-Tabelle), Print-CSS. Button 📖 in Orte-Toolbar. | Genealogisches Ortslexikon auf Knopfdruck; Steckbrief zeigt auf einen Blick Namenshäufigkeiten und Ereignis-Zeiträume. |
| v891 | **UI-Logik-Tests (T0-UI) — 9 neue Test-Blöcke (t)–(ab)** — `test-unit.js` erweitert um MiniDOM-Harness (~250 LOC, _makeMiniDOM mit classList/dataset/addEventListener/dispatchEvent/closest/querySelector) + UI-Loader (`_loadUI()`) der `ui-views-person.js`/`ui-views.js`/`ui-event-delegation.js`/`ui-lifecycle.js` + Slice `savePlaceObjects` aus `ui-forms.js` mit Stubs in den Eval-Scope hängt. AppState/UIState als const-Bindings via window-Brücke aus erster Eval-Phase exponiert. **Blöcke:** (t) ViewState 17 Tests (setCurrent exklusiver Fokus, getCurrent Validierung, viewstate-change Event, IDB-Persistenz); (u) `_activateDetailContainer` 18 Tests (dc-active Toggle, Scroll-Save/Restore, viewInit-Flag, Mobile-Pfad); (v) ClickMap 13 Tests (Action-Dispatch, stop-Sentinel, dataset-Fallbacks, closest-Bubble); (w) switchTab DirtyBit 11 Tests (renderTab nur bei dirty, vsReattach-Pfad, currentTab); (x) Lifecycle 10 Tests (visibilitychange <60s/>60s, pageshow BFCache-reload, pagehide persist); (y) ListSync 14 Tests (Person/Family/Place/Source .current-Klassen-Sync, no-list-no-throw); (z) Toast-Once 8 Tests (savePlaceObjects IDB-Fehler 1×-Toast, Once-Flag, Session-Reset); (aa) PLAC-Mode 11 Tests (initPlaceMode/togglePlaceMode/getPlaceFromForm free↔parts); (ab) FormSaveMerge 14 Tests (\_buildFormString periodengerecht, enclosure-Kette, applyStringPlaceLink Reimport-Erkennung, Cache-Invalidierung). +124 Tests, 296→**420** total. Roundtrip + CSP stabil. | Strukturelle Verriegelung der 4 Bug-Klassen aus v861–v890 P0–P6: ViewState-Konsistenz, Detail-Container-Skip-Pfad, Listen-Highlight-Sync, BFCache-Lifecycle. Regression-Erkennung jetzt automatisiert per `osascript -l JavaScript test-unit.js`, kein User-Test mehr nötig. |
...
| ID | Aufgabe | Details | Aufwand |
|---|---|---|---|
| **SCALE-TEST** | **Skalierungstest >10k Personen** ✅ *Messung abgeschlossen 2026-06-07* | Synthetisches 20k-GEDCOM erzeugt (`generate-scale-test.js`, 6,2 MB, 446k Zeilen, Roundtrip net_delta=0). Messung (`test-scale.js`) identifiziert **zwei klare Engpässe**: (1) **Sort ohne Cache** — `renderPersonList` ruft `[...persons].sort(localeCompare)` bei *jedem* Tab-Wechsel neu auf → **938 ms** bei 20k. (2) **Parser synchron im UI-Thread** → **688 ms** Cold-Start-Block. Details → SCALE-TEST-BEFUNDE.md. Fixes offen: **SORT-CACHE** (rendert am schnellsten Ergebnis) + **PARSER-WORKER** (Cold-Start nicht-blockierend). | M |
| ~~SORT-CACHE~~ | ~~**Sort-Cache für Personenliste**~~ | ✅ **Abgeschlossen sw v899** — `UIState._personSortCache = { mode, count, sorted }` in `ui-views-person.js`. Cache-Hit wenn `persons.length === allCount` (ungefiltert) + gleicher Sort-Modus. Invalidierung: `markChanged()` → `_invalidatePersonListCache()`, `_finishLoad` (GEDCOM + GRAMPS), `togglePersonSort()` via Mode-Mismatch. Tab-Wechsel nach erstem Render: 0 ms Sort-Overhead. | ~~S~~ |
| ~~PARSER-WORKER~~ | ~~**Parser in Web Worker**~~ | ✅ **Bereits implementiert** — `gedcom-worker.js` (`importScripts('gedcom-parser.js')`) wird in `storage-file.js:_processLoadedText` via `new Worker('gedcom-worker.js')` genutzt; Progress-Callbacks + Sync-Fallback vorhanden. Cold-Start ist bereits nicht-blockierend. | ~~L~~ |
...
**Handbuch-Stand: sw v858** *(veraltet — v859–v891 noch nicht dokumentiert: UX-Polish Orte-Steckbrief + View-Robustheit P0–P6 + Koord-Paar-Invariante + Koord-Löschen + po-gewinnt-immer + Ereignisliste/-gruppen + VS-Scroll-Reattach + UI-Logik-Tests T0-UI)*
