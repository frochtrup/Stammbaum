...
**Status:** 🟡 in Arbeit. **Phase 1 ✅ (sw v989):** `_migrateHofObjectsToPlaceObjects` + Tests, unverdrahtet. **Phase 2 Teil A ✅ (sw v990):** Migration verdrahtet (Storage-Schicht) + Dorf-Promotion + Reimport-Idempotenz, real verifiziert; +31 Hof-Unit-Tests (Gruppen (ae)+(af)). **Offen Teil B/C:** Writer-Notiz unter PLAC (`[Hof]`-Abbau), Zwei-Felder-Eingabe, `buildHofIndex`/Karte auf placeId, `hofObjects`-Sidecar entfernen, HANDBUCH.
