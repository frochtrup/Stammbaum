...
**Offen Teil 2c (Notiz-Hartschnitt):** Hof-**Notiz** bleibt bis dahin **dual-write** (Sidecar + Farm-PO), weil der GEDCOM-Notiz-Export noch über den `[Hof]`-Record läuft, der `db.hofObjects` liest. 2c stellt den Notiz-Export auf den Farm-PO um (Writer: `3 NOTE` unter PLAC **oder** `[Hof]`-Record aus Farm-PO-Notiz speisen) + Notiz-Roundtrip-Test als Gate, danach Notiz-Sidecar-Write-Stopp. Erst wenn Koords **und** Notiz nicht mehr in den Sidecar schreiben, kann der `stammbaum_hofobjects`-localStorage-Key (nach Altbestand-Migration) gelöscht werden. **Funktionsziel von ADR-026 ist erreicht** (geräteübergreifende Hof-Koords/-Notizen, sichtbare Binnenmigration, saubere Ort/Hof-Eingabetrennung).
