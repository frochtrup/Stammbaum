...
10. **Backlog-Status ↔ Code (mechanisch, ZUERST ausführen):**
    ...
    ```
    node .claude/skills/spec-lint/check-backlog.mjs            # L1–L14
    node .claude/skills/spec-lint/check-backlog.mjs --selftest  # prüft den Prüfer
    ```

    Wertet jede Zeile **beider Statusdateien** — `specs/v9/05-Backlog.md` (Hauptprogramm, `BL-n`) und `specs/v9/05a-Backlog-Orte-Editor.md` (Standalone-Orte-Editor, `OE-n`) — gegen den echten Code aus (Beleg-Syntax dort dokumentiert). **L1** „offen, aber Beleg trifft" und **L2** „gebaut, aber Beleg trifft nicht" sind Fehler (Exit 1); **L3** zählt Status-Wörter in den Specs 10–32 gegen eine Ratsche (seit BL-50 auf **0** — nie wieder anheben); **L4** warnt bei unauflösbaren Spec-Links; **L5** prüft, ob die Zeile im Abschnitt steht, der zu ihrem Status passt (`offen` → „Offene Punkte", `gebaut` → „Erledigte Punkte"); **L6** hält die Regel-Tabelle in `05-Backlog.md` und die Implementierung deckungsgleich; **L7** hält die Zahl der `[S]`/`[E]`-Bullets in Spec 20 gegen die Ratsche `SE_BULLETS` (30 seit BL-206/ADR-v9-148, zuvor 29 seit BL-51); **L8** prüft, ob der Beleg-Pfad einer Zeile in den Zuständigkeitsbereich ihrer Statusdatei fällt (`app-orte/`, `tests/orte/` → 05a, alles andere → 05); **L9** prüft die Tabellenstruktur selbst — auf jede Kopfzeile muss die Trennzeile `|---|` unmittelbar folgen. **L10** hält den Planungsteil der Priorisierung (Cluster-Tabelle + Wellen) gegen die Statusspalte — in beide Richtungen: Gebautes darf dort nicht als Arbeit stehen, Offenes muss vorkommen. **L11** meldet Zähl-Aussagen über den Realbestand ohne begleitenden Dateinamen gegen die Ratsche `L11_RATSCHE` (Warnung darunter, Fehler darüber) — der mechanisierbare Teil von ADR-v9-178; ausgelöst wird bei **Marker + Zahl**, nicht bei `N×` allein (das nackte Muster traf 79 statt 14 Zeilen). **L12** meldet ADR-Absätze in 04/04a, die einen Rest ankündigen, ohne eine `BL-`Nummer zu nennen (Ratsche `L12_RATSCHE`). **L13** verlangt für jede in `specs/v9/` benannte `INV-…`/`LP-…` eine Zeile in `tools/mutation/mutationen.mjs`. **L14** hält das Prosa-Budget einer `gebaut`-Zeile (200 Zeichen ohne Markdown-Links): mit ADR-Zeiger hart, ohne als Ratsche `L14_RATSCHE` — Regel 1 stand vorher nur als Prosa und war fünf Tage nach der Konsolidierung mit 20 KB zurück. Exit 0 = konsistent.
...
11. **04↔04a-Split (mechanisch, BL-161):**
    ...
    **Zweite Wortliste seit 2026-08-09 — der Verifikationsbefund.** Schritt 3a nennt ihn seit jeher gleichrangig neben Bau-Status und Testzahlen, erzwungen war aber nur die Nachtrag-Form; **20 solche Bullets** standen in `04` (`Verifikation`, `Rot-Proben`, `Eingecheckte Fixture`, `Am laufenden System belegt`, `Browser-Beleg`, `Beim Bau dazugelernt`), fünf allein in ADR-207/208. Sie sind ein Hauptgrund, warum `04` in zwei Wochen von 537 auf 1.078 KB wuchs, ohne dass ein Prüfer anschlug — dieselbe Lücke wie beim Backlog vor L14. Die Liste greift **nur am Leit-Label** und **ohne** freien Qualifizierer davor: gemeldet wird das Bullet, das einen BERICHT anführt, nicht die Entscheidungsklausel, die nach [ADR-v9-237](../../../specs/v9/04-Entscheidungslog.md#adr-v9-237) ihre Messung nennen MUSS. Deshalb kein `Verifiziert…`-Sammelmuster (`**Verifizierter Spec-Gap gegenüber dem v8-Oracle**`, ADR-36, ist Substanz) und kein `Befund`/`Messung`/`Bilanz` — beides am Bestand geprüft, nicht vermutet, und als Negativfälle im Selbsttest verankert (23 Fälle).

12. **Pflichtfeld „Berührte Prinzipien" (mechanisch, BL-308):**

    ```
    node .claude/skills/spec-lint/check-adr-prinzipien.mjs
    node .claude/skills/spec-lint/check-adr-prinzipien.mjs --selftest   # prüft den Prüfer
    ```

    Meldet ADRs, bei denen der Auslöser aus `decision-log` Schritt 4a greift und das Feld fehlt, gegen die Ratsche `RATSCHE` (Warnung darunter, Fehler darüber — Aufbau wie L11). Das Gegenstück zu Prüfung 11: dort wurde derselbe Umgehungsweg für Schritt 3a geschlossen, hier für 4a (das Feld stand nach seiner Einführung am 2026-08-02 in **0 von 212** ADRs; 210–213 entstanden per Hand-Edit, ohne den Skill je aufzurufen). **Der Auslöser ist bewusst eng und als Selbst-Erklärung des ADR gefasst** — ein `INV-…`-Kürzel im TITEL (11 Altfälle, alle inhaltlich Invarianten-setzend) oder ein Abschaffungs-Wort im `**Entscheidung`-/`**Konsequenz`-Bullet (1 Treffer: ADR-213). Die Position trägt die Regel, nicht das Wort: außerhalb dieser Bullets ist eine Abschaffung Erzählung über eine fremde (ADR-164/190/195). **Bewusst NICHT gebaut**, jeweils ausgezählt: „nennt der ADR ein Prinzip?" (die Bestätigungs-Form, die der Grundsatz gerade verwirft — sie hätte ADR-v9-197 durchgewinkt), „nennt `INV-` überhaupt" (138 von 212) und „`INV-` im Entscheidungs-Bullet" (72 von 212). Der Prüfer ist ein **Boden** unter der menschlichen Prüfung, kein Ersatz: von den sechs ADRs mit Feld löst er genau einen aus — „schafft einen Mechanismus ab" bleibt ein Urteil.
...
- **Zuerst die mechanischen Prüfungen 3/10/11/12 laufen lassen** (Sekunden) — erst danach die Mensch-assistierten Prüfungen 6/8/9.
