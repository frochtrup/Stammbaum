...
- **Angenommen (ungeprüft):** Tatsachenbehauptungen, die die Entscheidung trägt, aber nicht gemessen wurden — samt dem, was sie umstürzen würde (Schritt 4b). Weglassen, wenn alles belegt ist.
...
4a. **Ändert der ADR eine `INV-…` oder schafft er einen Mechanismus ab: das Pflichtfeld „Berührte Prinzipien" ausfüllen** (s. Eintragsformat) — beide Fragen wörtlich beantworten, bevor die Entscheidung steht, nicht danach als Begründung. Fällt dabei ein USP-Punkt auf, für den es keinen Versprechen-Test gibt, gehört er in denselben Zug oder in eine Backlog-Zeile. **Gilt auch beim direkten Hand-Edit an `04` (nicht nur beim Skill-Lauf)** — genau daran ist die Regel zuerst gescheitert: nach ihrer Einführung am 2026-08-02 entstanden 15 ADRs ohne das Feld, vier davon (210–213) per Hand direkt an der Datei, ohne den Skill je aufzurufen (BL-308). Der mechanische Boden darunter ist `node .claude/skills/spec-lint/check-adr-prinzipien.mjs` (spec-lint Prüfung 12); er erkennt nur die SELBST ERKLÄRTEN Fälle — `INV-…` im Titel oder ein Abschaffungs-Wort im `**Entscheidung`-/`**Konsequenz`-Bullet. „Schafft einen Mechanismus ab" bleibt ein Urteil, das kein Skript fällt: ein grüner Lauf ist kein Beleg, dass die Frage gestellt wurde.
4b. **Jede Klausel, die eine TATSACHE über den bestehenden Code oder den Datenbestand behauptet, nennt ihre Messung — oder markiert sich als Annahme.** Nicht „belege deine Meinung": es geht ausschließlich um Sätze, die etwas über die WELT sagen, das man nachsehen könnte („die Zeilen sind gleichförmig", „das kommt im Bestand nicht vor", „diese Fläche rendert flach", „das Flachlegen kostet nichts"). Erlaubte Formen sind zwei:

   - **mit Beleg** — Zahl plus Herkunft: `gemessen 34,1 vs. 51,1px (Browser, Realbestand `Testdateien/…`)`, `ausgezählt 7× (`grep`)`, `headless bei n=2.000`. Bei Zahlen aus dem Realbestand ist der Dateiname Pflicht ([ADR-v9-178](../../../specs/v9/04-Entscheidungslog.md#adr-v9-178), Backlog-Lint L11).
   - **als Annahme markiert** — `**Angenommen (ungeprüft):** …` samt dem, was sie umstürzen würde. Eine markierte Annahme ist kein Makel; eine unmarkierte ist eine Behauptung im Gewand einer Entscheidung.

   **Warum das ein eigener Schritt ist (BL-311, 2026-08-09):** [ADR-v9-235](../../../specs/v9/04-Entscheidungslog.md#adr-v9-235) enthielt DREI solche Klauseln, alle ohne Messung, alle falsch — und jede kostete einen Bauabschnitt:

   | Klausel | Behauptung | Wirklichkeit |
   |---|---|---|
   | Entscheidung 2 | „die Zeilen sind gleichförmig gebaut, die Abweichung liegt bei einzelnen Pixeln" | 34,1 gegen 51,1px — 50 % |
   | Entscheidung 4 | „ohne gemessene Höhe ist ‚alles rendern' die einzige richtige Antwort" | 3.083 Zeilen im ersten Takt; das Fenster senkte die Spitze gar nicht |
   | Entscheidung 1 (Nebensatz) | „bleibt richtig für Flächen, deren Trenner Zeilen IN der Liste sind (Personen-/Orte-Liste)" | die Personen-Trenner sind `position: sticky`; flachgelegt stapeln sie sich |

   Jede war in Minuten prüfbar. Der gemeinsame Nenner: eine Tatsachenbehauptung im Format einer **Entscheidung** wird nicht mehr als Behauptung gelesen — vom Autor am wenigsten. Deshalb die Prüfung an der Stelle, an der geschrieben wird, nicht an der, an der gebaut wird.

   **Bewusst nicht mechanisiert:** ob ein Satz eine Tatsache behauptet oder eine Absicht erklärt, ist ein Urteil; ein Blanko-Gate auf Zahlen ohne Dateinamen gibt es bereits (Backlog-Lint L11) und deckt nur die Teilmenge „Zahl aus dem Bestand" ab. Diese Regel deckt den größeren Teil: Behauptungen OHNE Zahl („gleichförmig", „kostet nichts", „kommt nicht vor") — und die sind die gefährlicheren, weil ihnen nicht einmal eine Zahl fehlt, an der jemand stutzen könnte.
