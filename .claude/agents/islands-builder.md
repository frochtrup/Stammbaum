...
- **Browser-Verifikation ist Pflicht:** Insel tatsächlich in einer Host-Seite rendern (`mcp__Claude_Browser__preview_start` + `computer{action:"screenshot"}`), Interaktion (Klick→Rezentrieren, Zoom/Pan wo zutreffend) real ausführen (`computer`/`find`), `read_console_messages` auf Fehler prüfen. **Lehre (2026-07-12, s. `ui-builder.md` für die volle Herleitung):** die `tools:`-Frontmatter referenzierte bis zu diesem Zeitpunkt den veralteten Namensraum `mcp__Claude_Preview__*` statt des aktuellen `mcp__Claude_Browser__*` — vor „Tool nicht verfügbar" im Bericht zuerst die oben gelisteten Namen gegen die tatsächlich sichtbaren Tools abgleichen. Nur bei echtem Tool-Fehlschlag explizit vermerken, nicht bei bloßer Bequemlichkeit auslassen.
