// Personen-Story-Abschnitte — geladen nach ui-story.js, nutzt window._storyShared
(function () {
  'use strict';
  ...
  const _occuPeriod         = _sh.occuPeriod;
  const _stripQuotes        = _sh.stripQuotes;
  const _partnerSpan        = _sh.partnerSpan;
  const _childSentence      = _sh.childSentence;
  const _mergeOccuSentence  = _sh.mergeOccu;
  const _mergeGradSentence  = _sh.mergeGrad;
  const _mergeEducSentence  = _sh.mergeEduc;
  const _mergeResiSentence  = _sh.mergeResi;
  const _mergeCareerSentence = _sh.mergeCareer;
  const _eventSentence      = _sh.eventSentence;
  const _loadMediaSrc       = _sh.loadMediaSrc;
  const _getParents         = _sh.getParents;

  // ── Foto-Embedding (async, Personen-Story) ──────────────────────────────────
...
<p>${_esc(p.note)}</p>
  ...
  function _storyAsHTML() {
    ...
    const mapUrl = _sh.getMapDataUrl();
    if (mapUrl) {
      body = body.replace(
        /<section[^>]*story-map[^>]*>[\s\S]*?<\/section>/,
        `<section class="story-section story-map"><img src="${mapUrl}" style="width:100%;max-height:280px;border-radius:6px;border:1px solid #ccc;display:block;object-fit:cover;"></section>`
      );
    } else {
...
@media print{
  ...
  .story-hero-img{max-height:160px !important}
  .story-map img{max-height:160px !important;width:100% !important;object-fit:cover}
  .story-diagram-wrap svg{max-height:150px !important}
  .story-family{page-break-inside:avoid}
...
<body>${body}</body>
  ...
  _sh.renderStory = _renderStory;
  _sh.embedPhotos = _embedPhotosAsync;
  _sh.storyAsHTML = _storyAsHTML;
