...
**PLAC-Hof-Pfade nur für hof-relevante Event-Typen** (sw v1042): Pfade A/A'/C feuern ausschließlich wenn `ev.type ∈ HOF_BOOTSTRAP_EVENT_TYPES = {RESI, PROP, CENS, OCCU}`. BIRT/DEAT/MARR/BURI/EDUC/GRAD/EVEN mit rich-PLAC „Schule für Bauwesen, Münster" oder „Krankenhaus St. Joseph, Münster" werden NIE zu Höfen interpretiert — semantisch korrekt, weil diese Events typischerweise an Schulen, Krankenhäusern, Kirchen oder Friedhöfen stattfinden. Pfad B (ADDR-basiert) bleibt offen für ALLE Event-Typen — explizite `ev.addr`-Eingabe ist User-Intent (BIRT mit `addr=Wall 33` ist bewusste Hof-Verknüpfung am bestehenden Hof). Symmetrisch zu Pfad B' (war seit Phase 4 schon beschränkt). Hof-Geburten/-Tode ohne ADDR werden via Review-Modal manuell zuweisbar.
