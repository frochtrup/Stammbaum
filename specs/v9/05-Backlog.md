...
|---|---|---|
| Ⓔ Orte, Karte & Geo | BL-310 (der Leerzustand zuerst — er trifft jeden neuen Bestand) · BL-230 (Ebenen-Wahl hängt sich in das gebaute B1-Bündel ein) | Karten-Insel, `MapLensView` |
| Ⓖ Navigation & Shell | BL-94 (Inhalt zuerst spezifizieren) | Shell / Navigation, `swipe-nav.ts`, Modal-Schalen |
| Ⓗ Import / Export / Dedup | BL-207 · BL-166 (vertagt) | Write-Back, Import-Vergleich, Dedup, Export-Pipe |
...
1. **Welle 4 — die drei Befunde der Stand-Bewertung (2026-08-04):** BL-310 · BL-312 · BL-311.
   Steht **vorn**, obwohl zuletzt angelegt: BL-310 ist die einzige `basis`-Zeile im ganzen
   offenen Bestand, und sie trifft jeden Nutzer beim ersten Blick auf die Karte. Die beiden
   anderen sind bewusst als **Messung vor Optimierung** geschnitten (Vorbild: das Perf-Gate,
   das zuerst verdrahtet und erst danach scharf gestellt wurde — die Zeilen nennen es je in
   ihrem Fertig-Zustand) — sie kosten je einen Testlauf, nicht einen Umbau, und beantworten erst danach die Frage,
   ob überhaupt etwas zu bauen ist. **Herkunft:** eigene Verifikation am laufenden Programm
   mit `Unsere Familie 2026.ged`, nicht aus dem Spec geschlossen; keiner der drei Punkte
   wurde von einem Gate gefangen, weil kein Gate an dieser Stelle misst.
...
|---|---|---|---|---|---|---|---|
| BL-310 | — | defekt | basis | **Die Karte-Lens benennt ihre Leere nicht** ([20 §1.9](20-Funktionen.md), [21 §5](21-UI-UX.md): „nie ein stiller Abbruch"). Im Default-Modus **Orte** zeigt die Karte bei einem Bestand ohne kuratierte Koordinaten eine leere Weltkarte — kein Text, kein Hinweis, kein Weg weiter. Verifiziert am 2026-08-04 im Browser mit `Unsere Familie 2026.ged` (3180 Personen) und frischer IndexedDB: 0 Marker, 0 erklärende Zeichen. **Der Mechanismus existiert schon eine Zeile weiter** — der Personen-Modus trägt „Keine Koordinaten für diese Person vorhanden" (`MapLensView.svelte`); dass derselbe Umstand im Nachbarmodus schweigt, ist der INV-UI-4-Fall in der Negativform: ein Muster, das an mehreren Stellen dieselbe Rolle spielt, ist nur an einer umgesetzt. **Warum das der Regelfall und nicht der Randfall ist:** der Orts-Seed erzeugt beim Import unangereicherte PlaceObjects ohne eigene Koordinaten ([ADR-v9-28](04-Entscheidungslog.md#adr-v9-28)) — direkt nach dem ersten Laden ist die Karte also IMMER leer, und genau dort trifft der Nutzer sie zuerst. **Fertig-Zustand:** eine reine Funktion nennt den GRUND (gar keine Orte ⇄ Orte vorhanden, aber keiner mit Koordinaten) statt nur „leer", und der zweite Fall führt sichtbar auf den vorhandenen Batch-Geocoding-Weg ([BL-130](#erledigte-punkte), [20 §1.7](20-Funktionen.md)). Gilt für beide Rendering-Pfade (Leaflet + SVG-Fallback), Offline-Parität wie bei [ADR-v9-154](04-Entscheidungslog.md#adr-v9-154). **Keine zweite Badge-/Glyphen-Fundstelle** ([21 §6h](21-UI-UX.md)) — ein Satz auf der leeren Fläche, kein Dauer-Element in der Kopfzeile | [20 §1.9](20-Funktionen.md), [21 §5](21-UI-UX.md) | `sym:mapEmptyReason` | offen |
