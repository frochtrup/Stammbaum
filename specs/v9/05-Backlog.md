...
| Ⓙ Bearbeitung & Formulare | BL-233 · BL-234 (unabhängig) · BL-232 (Persistenz hängt sich in das B1-Bündel ein) · BL-334 (Meldungs-Kanäle) | `ui/shell/StatusNotice.svelte`, `ui/views/person`, `ui/views/place`, `ui/views/hof`, `ui/views/media`, `ui/shell` (Formulare, `SourceCitationRow`, Ablage), `tests/ui/touch-target.test.ts` |
| Ⓝ Doku-Disziplin | BL-322 (zwei Abarbeitungslisten, unabhängig voneinander) | `specs/v9/`, `.claude/skills/spec-lint/` |
| Ⓞ Zeitachse der Orte | BL-326 (die Wahlregel, unabhängig und noch unentschieden) · BL-332 (Ableiten statt Prüfen) | `core/places/types.ts`, `core/places/place-registry.ts`, `core/places/gov.ts`, `services/places/types.ts`, `ui/views/place/PlaceEnclosureEditModal.svelte`, `ui/views/place/PlaceNamesSection.svelte`, `ui/views/hof/HofDetail.svelte` |
...
3. **Welle 2 — eigenständige Kür:** BL-230 (die Ebenen-Wahl hängt sich in das gebaute
   B1-Bündel ein) · BL-207 · BL-233 · BL-234 · BL-334 (die Meldungs-Kanäle ziehen auf den
...
4. **Welle 6 — die Zeitachse der Orte (2026-08-09, [ADR-v9-243](04-Entscheidungslog.md#adr-v9-243)):**
   BL-326 · BL-332. Drei Schritte dieser Welle — der fehlende Überlappungs-Hinweis, die
   Tagesgenauigkeit selbst und die dritte, dabei übersehene Eingabefläche — sind am
   2026-08-09 gebaut und stehen deshalb nicht mehr hier. **BL-326 bewusst zuletzt und unabhängig:** seine Entscheidung
...
| BL-332 | — | feature | kür | **Das Jahr beim Laden aus dem Stichtag ABLEITEN, statt es nur zu prüfen** — ein datierter Eintrag trägt zwei Hälften (`from`/`to` als Jahr, `fromDate`/`toDate` als Tag). Die Eingabe schreibt beide aus EINEM Feld, aber eine anderswo entstandene oder von Hand bearbeitete Datei kann sie auseinanderlaufen lassen: an `Testdateien/orte-5.json` (rev 314) waren es **16 von 1506** datierten Einträgen, alle auf einer Jahresgrenze und genau ein Jahr daneben ([ADR-v9-246](04-Entscheidungslog.md#adr-v9-246) Verworfen c). `parsePlacesFileWrapper` ist der EINE Punkt, an dem `to` aus `toDate` abgeleitet werden könnte — danach ist Inkongruenz strukturell unmöglich statt nur bewacht. **Der Preis ist die Sichtbarkeit:** die Ableitung heilt still, und der Wächter, der diese 16 überhaupt zutage gefördert hat, hätte nichts mehr zu melden. **Fertig-Zustand:** eine Entscheidung per ADR; wird abgeleitet, ein Test, der eine inkongruente Datei lädt und das Ergebnis belegt | [11 §1](11-Orte-Hoefe-Identitaet.md), [30 §2.1](30-NFR-und-Persistenz.md), [ADR-v9-243](04-Entscheidungslog.md#adr-v9-243), [ADR-v9-246](04-Entscheidungslog.md#adr-v9-246) | `sym:leiteGrenzjahrAb` | offen |
