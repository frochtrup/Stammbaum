...
| BL-140 | S | feature | kür | GRAMPS→Modell Lese-Projektion (Stufe 1, ADR-v9-114 D1/D2/D4/D5): Ereignisse (`<eventref role>`→Main-Slots/`events[]`, geteilte Events by-reference, Nicht-Owner-Rollen→`Association`), Daten (`dateval`/`daterange`/`datespan`/`datestr`→GEDCOM-Datumsstring+`datePhrase`), Zitate (zweistufig `citationref→citation→sourceref`, `confidence`→`quay=min(·,3)`). Heute projiziert `parseXMLText` nur Name/Geschlecht/Links — der GRAMPS-Import ist im Editor leer | [13 §6](13-Interop-Roundtrip.md), [10 §5](10-Domaenenmodell.md), [ADR-v9-114](04-Entscheidungslog.md#adr-v9-114) | `test:tests/core/gramps-projection.test.ts` | offen |
