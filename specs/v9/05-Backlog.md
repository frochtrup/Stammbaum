...
| BL-159 | S | test | kür | Cross-Family (5/6, ADR-v9-127): RT-4-Gate `A→Modell→B→Modell'` ⇒ `Modell≈Modell'` in beide Richtungen, an Klein- + Realdaten-Fixtures. **Enthält modelEquiv-Härtung** (Befund BL-157/158): Personen-Signatur über Namens*teile* (given/surname/sex/Daten) statt des zusammengesetzten `name`-Strings — der ist unter Cross-Family-Namensrekonstruktion zerbrechlich (1431/2795 Personen roh nicht paarbar); der Cross-Namespace-Resolver-Bug (b-Seite mit a-Resolver) ist bereits in BL-157 gefixt | [13 §1 RT-4](13-Interop-Roundtrip.md), [ADR-v9-127](04-Entscheidungslog.md#adr-v9-127) | `test:tests/roundtrip/cross-family.test.ts` | offen |
