...
| BL-126 | S | feature | kür | Medien-Verwaltungs-**UI**: Kachelgalerie · Medium-Detail (globale vs. referenz-spezifische Felder) · Referenzliste mit `Picker`-Verknüpfen. **Kern gebaut** (ADR-v9-124/125): `Media` als Top-Level-Entität (globaler Titel + Per-Ref-Override) + GEDCOM-Parse/Write-Back (inkl. Record-Edit) + GRAMPS-Projektion + Roundtrip (net_delta=0). UI-Phase-Rest: GRAMPS-Medien-Write-Back + Citation-Ebene-`objref` (heute Passthrough) | [20 §1.4/§1.5](20-Funktionen.md), [10 §4](10-Domaenenmodell.md) | `datei:ui/views/media/MediaGallery.svelte` | offen |
...
| BL-154 | E | feature | kür | **Cross-Family-Emission (Epic-Umbrella)** GEDCOM↔GRAMPS aus EINEM Modell — Plan/Zerlegung in [ADR-v9-127](04-Entscheidungslog.md#adr-v9-127); buildbare Teilstücke BL-155…160. „Fertig" = der eine Cross-Convert-Pfad steht (Native/LP-1 unangetastet) | [14 §5](14-Dateihandling.md), [13 §1 RT-4](13-Interop-Roundtrip.md), [ADR-v9-127](04-Entscheidungslog.md#adr-v9-127) | `sym:convertDocFormat` | offen |
| BL-160 | S | feature | kür | Cross-Family (6/6, ADR-v9-127): Export-Wiring — `db` in JEDES Format exportierbar (nicht mehr ans Quell-Doc gebunden); Export-Fläche bietet alle Formate (GRAMPS nicht nur bei geladenem `.gramps`, ADR-v9-113 E5 überholt); Cross-Export erzwingt Download/Suffix, nie in-place | [14 §3.2](14-Dateihandling.md), [20 §1.2](20-Funktionen.md), [ADR-v9-127](04-Entscheidungslog.md#adr-v9-127) | `sym:exportCrossFamily` | offen |
| BL-162 | S | hygiene | kür | Coverage-Audit einen NENNER geben: das BL-155-Audit misst nur `kommt vor` in zwei Bestandsdateien (`_coverage-audit.census.test.ts`), nicht den Möglichkeitsraum der Spec — „0 Vorkommen"-Zeilen (LDS-Ordinanzen, `ASSO`-Nicht-Owner, GRAMPS `tagref`/`lds_ord`/Witness-`eventref`) heißen „nicht in diesen Dateien", nicht „nicht in der Spec" (ADR-v9-124-Lehre). Spec-getriebener Vollständigkeits-Pass: Record-/Subtag-Universum aus der **öffentlichen** GEDCOM-5.5.1/7.0-Def + dem GRAMPS-XML-DTD enumerieren, Modell-Abdeckung dagegen halten (jedes Konstrukt: modelliert ODER dokumentiert passthrough). Gate für die Cross-Family-Robustheit (BL-157/158) gegen FREMDE, spec-konforme Dateien | [13 §1.1](13-Interop-Roundtrip.md), [ADR-v9-127](04-Entscheidungslog.md#adr-v9-127), [ADR-v9-124](04-Entscheidungslog.md#adr-v9-124) | `test:tests/core/coverage-spec.test.ts` | offen |
