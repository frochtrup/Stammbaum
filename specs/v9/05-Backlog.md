...
| BL-154 | E | feature | kür | **Cross-Family-Emission** (GEDCOM↔GRAMPS aus EINEM Modell, ADR-v9-126): (a) Modell→Vollbaum-Synthese je Format von Grund auf (Emit-Primitiven existieren, Orchestrierung fehlt); (b) Normalisierungsschicht `FORM`↔MIME, MEDI-Enum, Xref-Remapping `@I1@`↔`I0001` (Datum/Ort haben sie schon); (c) Coverage-Audit der heute passthrough-only-Felder; (d) Cross-Family-Gate `A→Modell→B→Modell'` (Modell-Äquivalenz statt Byte). Native Passthrough (LP-1) bleibt unangetastet | [14 §5](14-Dateihandling.md), [13](13-Interop-Roundtrip.md), [ADR-v9-126](04-Entscheidungslog.md#adr-v9-126) | `datei:core/interop/model-emit.ts` | offen |
