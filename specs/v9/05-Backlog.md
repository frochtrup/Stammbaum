...
| Ⓙ Bearbeitung & Formulare | BL-233 · **BL-353 → BL-354** (die Erfassungs-Vorlagen; Kern-Primitive und Erfassungs-Fläche sind gebaut — offen sind nur noch Verwaltung/Builder und der JSON-Transport) · BL-334 (Meldungs-Kanäle) | `ui/shell/StatusNotice.svelte`, `ui/views/person`, `ui/views/place`, `ui/views/hof`, `ui/views/media`, `ui/shell` (Formulare, `SourceCitationRow`, Ablage, `EventEditModal`), `core/model/entry-templates.ts`, `services/app-data/`, `tests/ui/touch-target.test.ts` |
...
5. **Welle 3 — Infra & Hygiene (opportunistisch):**
   ...
   **BL-353 · BL-354** (die Erfassungs-Vorlagen, [ADR-v9-264](04-Entscheidungslog.md#adr-v9-264) —
   in dieser Reihenfolge; Kern-Primitive und Erfassungs-Fläche sind gebaut, offen sind
   Verwaltung/Builder und der JSON-Transport) · BL-322 (die beiden
