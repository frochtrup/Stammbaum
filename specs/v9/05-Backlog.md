...
| Ⓖ Navigation & Shell | BL-319 (ViewState-Slots fürs Dashboard) · BL-94 (Inhalt zuerst spezifizieren) | Shell / Navigation, `swipe-nav.ts`, Modal-Schalen |
...
1. **Welle 5 — was ein Nutzer heute trifft:** BL-319 · BL-318 · BL-315. BL-319 zuerst: die Ursache ist bekannt und der Weg vorgezeichnet. Beide sind Befunde am laufenden Programm, beide warten auf eine Messung: Steht vorn, weil es die einzige
...
|---|---|---|---|---|---|---|---|
| BL-319 | — | defekt | basis | **Der Zustand des Qualitäts-Dashboards überlebt keine Navigation** — Nutzer-Befund 2026-08-08: Filter setzen, zur Person mit dem Hinweis wechseln, zurück — der Filter steht wieder auf dem Standard. **Ursache am Code:** `focusFilter`, `showReport`, `reportScope` und `branchSelection` sind komponenten-lokaler `$state` (`ui/views/quality/QualityDashboard.svelte`); der Tab-Wechsel unmountet die Komponente, und mit ihr stirbt jedes der vier Felder — bei der Rückkehr startet sie auf `DEFAULT_FOCUS`. Betroffen ist also nicht nur der Filter, sondern auch ein offener Bericht samt Umfang und eine gewählte Ast-Auswahl. **Kein Sonderfall, sondern eine fehlende Anbindung:** navigationsfeste Zustände gehören in den `ViewState`; für die Zeitleisten-Vergleichsliste hält [20 §1.10](20-Funktionen.md) das ausdrücklich fest („sie überlebt das Wegnavigieren"), das Dashboard nutzt das Muster nur nicht. **Fertig-Zustand:** eigene ViewState-Slots je Zustand, INV-VS gewahrt (keine Kopplung an `person` — [ADR-v9-102](04-Entscheidungslog.md#adr-v9-102)), und ein Test, der den ganzen Weg geht: setzen → wegnavigieren → zurück → **hinsehen** (nicht den Speicher abfragen) | [20 §1.11g](20-Funktionen.md), [21 §5](21-UI-UX.md), [ADR-v9-102](04-Entscheidungslog.md#adr-v9-102) | `txt:qualityFocus@ui/shell/view-state.svelte.ts` | offen |
...
| BL-192 | S | feature | kür | **Geo-Befunde im Qualitäts-Dashboard sichtbar** | [20 §1.7](20-Funktionen.md), [20 §3](20-Funktionen.md) | `txt:reportScope@ui/views/quality/QualityDashboard.svelte` | gebaut |
