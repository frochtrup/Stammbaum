...
| Ⓙ Bearbeitung & Formulare | BL-233 · BL-232 (Persistenz hängt sich in das B1-Bündel ein) · BL-334 (Meldungs-Kanäle) | `ui/shell/StatusNotice.svelte`, `ui/views/person`, `ui/views/place`, `ui/views/hof`, `ui/views/media`, `ui/shell` (Formulare, `SourceCitationRow`, Ablage), `tests/ui/touch-target.test.ts` |
