...
| Ⓞ Zeitachse der Orte | BL-325 (der fehlende ⚠ — zuerst, macht das Problem überhaupt sichtbar) · BL-324 (Tagesgenauigkeit) · BL-326 (die Sprachachse, unabhängig und noch unentschieden) | `core/places/types.ts`, `core/places/place-registry.ts`, `core/places/gov.ts`, `services/places/types.ts`, `ui/views/place/PlaceEnclosureEditModal.svelte`, `ui/views/place/PlaceNamesSection.svelte` |
...
4. **Welle 6 — die Zeitachse der Orte (2026-08-09, [ADR-v9-243](04-Entscheidungslog.md#adr-v9-243)):**
   BL-325 · BL-324 · BL-326. Die Reihenfolge ist keine Bequemlichkeit: **BL-325 zuerst**,
   weil der fehlende ⚠ die 433 Randberührungen überhaupt erst sichtbar macht — ohne ihn
   baut BL-324 gegen ein Problem, das niemand sehen kann, und der Nutzer hätte keinen Weg,
   die Stichtage dort zu pflegen, wo sie fehlen. **BL-326 zuletzt und unabhängig:** es ist
   die einzige der drei Zeilen, deren Entscheidung noch aussteht, und sie berührt mit der
   Match-Namensmenge (§4.2) die Ereignis-Auflösung — das ist keine Anzeige-Frage.
   **Herkunft:** Nutzer-Vorgabe „tagegenau abbildbar", gemessen am maßgeblichen Bestand
   (`Testdateien/orte-2.json` rev 277 + `Testdateien/Unsere Familie 2026.ged`), nicht aus
   dem Spec geschlossen.
5. **Welle 3 — Infra & Hygiene (opportunistisch):**
   BL-232 (Templates hängen sich in das gebaute B1-Bündel ein) · BL-322 (die beiden
...
|---|---|---|---|---|---|---|---|
| BL-324 | — | feature | usp | **Tagegenaue Gültigkeitsdaten für Verwaltungshierarchie und Ortsnamen** — Umsetzung von [ADR-v9-243](04-Entscheidungslog.md#adr-v9-243): `fromDate`/`toDate` (GEDCOM-Datumsstring) additiv an `Dated`/`DatedRef`, zweistufiger Vergleich (Tag nur wenn beide Seiten ihn tragen, sonst Jahr) in `resolveAsOf`/`enclosureWinnerAsOf`/`enclosureIdsAsOf`/`enclosureChainAsOf`/`chainCompatibleAnyPath`/`findByAddr`/`buildPlacForGedcom`, Ereignisseite über `eventDate` neben `eventYear`; `govYear()` hört auf, Monat/Tag zu verwerfen; `orte.json` auf `schemaVersion: 2` (schützt über das vorhandene Read-Only-Gate vor stillem Verlust beim Sync mit älterem Stand); Editier-Flächen `PlaceEnclosureEditModal`/`PlaceNamesSection` von Jahres- auf Datumsfelder. App-privat — kein GEDCOM/GRAMPS-Schreibpfad. **Fertig-Zustand:** ein Ereignis mit Tagesdatum an einer Randberührung (`[…1810] × [1810…]`) löst auf die periodenrichtige Kette auf, ein jahrgenaues Ereignis unverändert wie heute | [11 §1](11-Orte-Hoefe-Identitaet.md), [11 §5](11-Orte-Hoefe-Identitaet.md), [30 §2.1](30-NFR-und-Persistenz.md), [ADR-v9-243](04-Entscheidungslog.md#adr-v9-243) | `sym:fromDate` | offen |
| BL-325 | — | defekt | basis | **Der ⚠-Hinweis auf überlappende Perioden ist spezifiziert, aber nicht gebaut** — [11 §5](11-Orte-Hoefe-Identitaet.md) sagt, `buildPlacForGedcom`/`enclosureChainAsOf` markierten den Fall im Ort-Steckbrief; im Code gibt es nur `EnclosureMeta.truncated` (Kettenabbruch). Am maßgeblichen Bestand (`Testdateien/orte-2.json`, rev 277) sind **433 `enclosedBy`-Paare** überlappend und **94 Ereignisse** betroffen — alle werden heute stumm per Tie-Break aufgelöst. **Fertig-Zustand:** der Steckbrief zeigt den Hinweis, ein Test deckt ihn ab | [11 §5](11-Orte-Hoefe-Identitaet.md), [ADR-v9-243](04-Entscheidungslog.md#adr-v9-243) | `sym:ueberlappt` | offen |
| BL-326 | — | feature | kür | **74 `pnames`-Überlappungen sind Parallelität, keine Datierung** — gemessen an `Testdateien/orte-2.json` (rev 277): „Guben [1235…]" neben „Gubin [1945…]", „Département de la Lippe [1811–1813]" neben „Lippe-Departement [1811–1813]". Zwei gleichzeitig gültige Namen desselben Orts gehören auf die **Sprachachse** (`translations`, BL-59), nicht auf die Zeitachse; [ADR-v9-243](04-Entscheidungslog.md#adr-v9-243) Entscheidung 5 klammert sie ausdrücklich aus, weil Tagesgenauigkeit sie nicht löst. **Offen ist die Frage selbst:** umhängen (verändert die Match-Namensmenge aus §4.2 und damit die Ereignis-Auflösung — nicht folgenlos), nebeneinander zulassen und nur die Projektion eindeutig machen, oder als legitim akzeptieren. **Fertig-Zustand:** eine Entscheidung per ADR, samt Messung, wie viele der 174 heute namens-mehrdeutigen Ereignisse sie bewegt | [11 §1](11-Orte-Hoefe-Identitaet.md), [11 §4.2](11-Orte-Hoefe-Identitaet.md), [ADR-v9-243](04-Entscheidungslog.md#adr-v9-243) | `test:tests/core/pname-parallelitaet.test.ts` | offen |
