...
| BL-136 | — | defekt | usp | GRAMPS-Projektion mischt zwei Schlüsselarten: `db.individuals` ist unter der `id` abgelegt (`I0001`), `family.husband/wife/children` tragen die `hlink`-HANDLES aus der Datei. Gemessen an `Unsere Familie.gramps`: **0 von 3804** Familien-Referenzen lassen sich in `db.individuals` auflösen — ein GRAMPS-Import hätte keine einzige sichtbare Verwandtschaft. Fällt heute nicht auf, weil der GRAMPS-Import gar nicht in der UI hängt (`ImportButton`: „nicht Teil dieser Scheibe"). Beim Write-Back (BL-80) nur notdürftig überbrückt (`alsHandle`) | [13 §6](13-Interop-Roundtrip.md), [10 §2](10-Domaenenmodell.md) | `test:tests/core/gramps-references.test.ts` | offen |
| BL-119 | S | feature | basis | Export-Fläche im „Datei"-Eintrag (kein eigenes Nav-Ziel): Formatwahl 5.5.1/Strict/GED7 + Anonymisierungs-Checkbox mit Zähler „N von M Personen werden geschwärzt", über dieselbe `save-action.ts`-Ebene wie Knopf und ⌘S. `ExportFormat` hat in `ui/`/`app/` bis dahin keinen einzigen Aufrufer, der Speichern-Knopf fährt ausschließlich 5.5.1 in-place. Setzt BL-138 voraus; GRAMPS bewusst ausgeklammert (BL-139) | [20 §1.2](20-Funktionen.md), [14 §3](14-Dateihandling.md), [ADR-v9-113](04-Entscheidungslog.md#adr-v9-113) | `datei:ui/views/export/ExportView.svelte` | offen |
| BL-138 | — | defekt | basis | Anonymisierter Export ist unbrauchbar und nicht verdrahtet: (a) `buildLivingSet` propagiert die BFS ohne `dead`-Bremse durch datierte Verstorbene — gemessen 2767 von 2795 Personen als lebend (Orakel: 689, Faktor 4); (b) `anonymizeIndi` hat null Produktions-Aufrufer, es fehlt die Dokument-Ebene über die `roots` und der orthogonale Rohr-Schalter `anonymizeReferenceYear` samt erzwungenem `_anon`-Suffix/Download auch bei 5.5.1 mit Handle; (c) FAM-Ereignisdetails lebender Paare bleiben stehen (265 `MARR`-Daten) | [13 §7](13-Interop-Roundtrip.md), [14 §3.2](14-Dateihandling.md), [ADR-v9-113](04-Entscheidungslog.md#adr-v9-113) | `test:tests/core/interop-anonymize-doc.test.ts` | offen |
| BL-139 | S | feature | kür | GRAMPS-Export aus der UI erreichbar machen. Heute hält die App nie ein `grampsDoc` (kein `parseXMLText`-Aufruf in `ui/`/`app/`), und der Cross-Export GEDCOM→GRAMPS über das leere Gerüst liefert gemessen 0 Ereignisse, 0 Orte, 0 Zitate und kein Datum (845 KB gegen 5,7 MB echtes GRAMPS). Bündelt GRAMPS-Import in der UI, BL-136 und die echte Cross-Projektion — ein Menüpunkt allein wäre stiller Totalverlust | [13 §6](13-Interop-Roundtrip.md), [14 §3.2](14-Dateihandling.md), [ADR-v9-113](04-Entscheidungslog.md#adr-v9-113) | `txt:gramps@ui/views/export/ExportView.svelte` | offen |
