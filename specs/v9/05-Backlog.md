...
| Ⓙ Bearbeitung & Formulare | BL-233 · **BL-232 → BL-352 → BL-353 → BL-354** (die Erfassungs-Vorlagen in dieser Reihenfolge: BL-232 ist die Primitive, die drei übrigen sind ihre Konsumenten; die Persistenz hängt sich in das B1-Bündel ein) · BL-334 (Meldungs-Kanäle) | `ui/shell/StatusNotice.svelte`, `ui/views/person`, `ui/views/place`, `ui/views/hof`, `ui/views/media`, `ui/shell` (Formulare, `SourceCitationRow`, Ablage, `EventEditModal`), `core/model/entry-templates.ts`, `services/app-data/`, `tests/ui/touch-target.test.ts` |
...
2. **Welle 4 — die Befunde der Stand-Bewertung (2026-08-04):** BL-312 · BL-313
...
3. **Welle 2 — eigenständige Kür:** BL-230 (die Ebenen-Wahl hängt sich in das gebaute
   B1-Bündel ein) · BL-207 · BL-233 · BL-334 (die Meldungs-Kanäle ziehen auf den
   Baustein nach — Nacharbeit, keine Voraussetzung von irgendetwas). **Drei Zeilen dieser Welle sind am 2026-08-03 gebaut**
...
5. **Welle 3 — Infra & Hygiene (opportunistisch):**
   ...
   Reconciliation ohnehin angefasst wird, [ADR-v9-244](04-Entscheidungslog.md#adr-v9-244)) ·
   **BL-232 · BL-352 · BL-353 · BL-354** (die Erfassungs-Vorlagen, [ADR-v9-264](04-Entscheidungslog.md#adr-v9-264) —
   in dieser Reihenfolge und nicht beliebig teilbar: BL-232 trägt Modell, Persistenz im
   gebauten B1-Bündel und das Anwenden-Kommando, die drei übrigen bauen darauf auf;
   opportunistisch ist hier nur der EINSTIEG, nicht die Folge) · BL-322 (die beiden
   Abarbeitungslisten der Doku-Wächter — je Zeile ein Umzug nach [04a](04a-Chronik.md),
   beliebig teilbar, deshalb opportunistisch: am billigsten dann, wenn die betroffene
   Stelle ohnehin angefasst wird, [ADR-v9-240](04-Entscheidungslog.md#adr-v9-240)).
...
| BL-334 | — | hygiene | kür | **14 handgebaute Meldungs-Kanäle ziehen auf `StatusNotice` nach** — der Baustein aus [ADR-v9-247](04-Entscheidungslog.md#adr-v9-247) (Text · 12 s Frist · ✕) trägt heute nur den Hinweis-Kanal der Schale. Daneben stehen 14 eigene Fassungen desselben Musters (`let notice = $state('')` in `UndoControls`, `SaveButton`, `ExportView`, `SettingsView`, `GovImportSection`, `ImportButton`, `PlacesFileButtons` ×2, `AppDataFileButtons` ×2, `HofDetail` ×2, `PlaceNamesSection`, `PlaceEnclosureEditModal`) — jede mit eigener Optik, die meisten ohne Frist und ohne Ausgang. Bewusst NICHT im Zug von ADR-v9-247 umgebaut: aus einem Bugfix sollte keine Sanierung quer durch die Schale werden. **Fertig-Zustand:** die Kanäle nutzen den Baustein oder ihre Abweichung ist im Code begründet (ein Formular-Feldfehler wie `grenzFehler` ist etwas anderes als eine transiente Rückmeldung); ein Wächter zählt die verbliebenen Eigenbauten gegen eine Ratsche | [21 §6](21-UI-UX.md), [ADR-v9-247](04-Entscheidungslog.md#adr-v9-247) | `sym:statusNoticeRatsche` | offen |
...
| BL-322 | — | hygiene | kür | **Die zwei Abarbeitungslisten der Doku-Wächter leerlaufen lassen** ([ADR-v9-240](04-Entscheidungslog.md#adr-v9-240)): (a) **12 KB datierte Vorgeschichte** stehen noch in den Basis-Specs [10](10-Domaenenmodell.md)/[11](11-Orte-Hoefe-Identitaet.md)/[12](12-Forschungsdaten.md)/[13](13-Interop-Roundtrip.md)/[30](30-NFR-und-Persistenz.md)/[31](31-Dev-Umgebung.md) — 20/21/32 sind bereinigt, die übrigen bewusst nicht (Nutzer-Entscheidung 2026-07-12: kein Rundumschlag, nur beim Anfassen). (b) **36 erledigte Backlog-Zeilen ohne ADR-Anker** über dem L14-Budget: ihr Inhalt zieht nach [04a](04a-Chronik.md) um, dann kürzt die Zeile auf Titel + Zeiger. **Fertig-Zustand:** die L14-Ratsche steht unter 36 und in den sechs genannten Specs steht keine datierte Vorgeschichte mehr; beides misst der Lint, nicht eine Durchsicht | [05 Regel 1](05-Backlog.md), [ADR-v9-240](04-Entscheidungslog.md#adr-v9-240) | `!txt:L14_RATSCHE = 36@spec:.claude/skills/spec-lint/check-backlog.mjs` | offen |
...
| BL-232 | — | feature | kür | **Erfassungs-Vorlagen — Modell, Persistenz, Anwenden-Kommando** ([ADR-v9-264](04-Entscheidungslog.md#adr-v9-264)). Vorlagen-Typ + die drei mitgelieferten Standard-Vorlagen als Daten in `core/model/entry-templates.ts` (Präzedenz `source-templates.ts`); `entryTemplates`-Abschnitt im **gebauten** B1-Bündel ([BL-180](#erledigte-punkte)) samt Verallgemeinerung des heutigen `if (key === 'projects')`-Sammlungs-Merges auf den zweiten Sammlungs-Abschnitt; `applyEntryTemplate` legt alle Personen/Familien/Bindungen in EINEM `editDatabase`-Durchlauf an (ein Undo-Schritt, INV-P3 gewahrt); `findFamilyFor` verwendet passende Familien wieder und fragt bei ≥2 Kandidaten, statt zu raten. **Voraussetzung von BL-352/353/354.** **Fertig-Zustand:** ein Test, der eine Vorlage mit Eltern- und Ehefamilie anwendet und danach EINEN Undo-Schritt zum Ausgangszustand zurückführt | [20 §2](20-Funktionen.md), [30 §2.2](30-NFR-und-Persistenz.md), [10 §5.1](10-Domaenenmodell.md) | `sym:applyEntryTemplate` | offen |
| BL-352 | — | feature | kür | **Erfassungs-Fläche der Vorlagen** ([ADR-v9-264](04-Entscheidungslog.md#adr-v9-264) Entscheidungen 1/5/10): rendert die Felder einer Vorlage aus den vorhandenen Struktureingaben, hält den Entwurf bis „Speichern", verknüpft Rollen per Live-Dubletten-Treffer (`scorePersonPair`, entprellt — kein zweiter Vergleicher), meldet die Plausibilität über Einzelregeln aus `RULES_BY_ID` auf den berührten Datensätzen und bleibt für die Serie offen. **Enthält die eine echte Baustein-Lücke:** die Datumszeile (Qualifier + T/M/J) steckt heute als Markup inline in `ui/shell/EventEditModal.svelte` — sie wird als `EventDateFields.svelte` extrahiert, und das Modal konsumiert sie danach selbst (INV-UI-4, Geschwister-Stelle im selben Zug). **Fertig-Zustand:** ein Komponententest, der eine Vorlage ausfüllt, einen Dubletten-Treffer verknüpft und speichert — plus der Nachweis, dass `EventEditModal` dieselbe Datumskomponente rendert | [20 §2](20-Funktionen.md), [21 §6a](21-UI-UX.md) | `sym:EntryTemplateCapture` | offen |
| BL-353 | — | feature | kür | **Vorlagen-Verwaltung und Builder** ([ADR-v9-264](04-Entscheidungslog.md#adr-v9-264) Entscheidungen 2/3/9): Liste zum Anlegen/Kopieren/Auswählen/Löschen, Rollen- und Feld-Editor (Feldwahl über die Modell-Feldnamen und `EventTypeMenu`, Vorbelegung je Feld mit versteckt/gesperrt), Quellen-Vorbelegung über `SourcePicker` mit Fingerabdruck. Einstieg ist der **dritte Knopf in der vorhandenen `Werkzeuge`-Disclosure** der Personenliste — kein Nav-Ziel, kein zusätzliches Dauer-Bedienelement. **Fertig-Zustand:** eine im Builder gebaute Vorlage ist von einer mitgelieferten nicht unterscheidbar (gleicher Typ, gleicher Anwenden-Pfad), und die Fläche ist bei 375px **gemessen**, nicht geschätzt (INV-UI-11) | [20 §2](20-Funktionen.md), [21 §6h](21-UI-UX.md) | `sym:EntryTemplateView` | offen |
| BL-354 | — | feature | kür | **JSON-Ex-/Import einzelner Erfassungs-Vorlagen** ([20 §2](20-Funktionen.md)). App-privat, reist nicht mit der Genealogie-Datei; der laufende Bestand reist ohnehin im `app-data.json`-Bündel — dies ist der Weg, EINE Vorlage weiterzugeben. Über dieselben `FileService`-Adapter wie der Bündel-Transport, kein eigener Pfad (INV-FILE-3). **Fertig-Zustand:** eine exportierte Vorlage wird reimportiert und ergibt denselben Typ, ohne den vorhandenen Bestand zu ersetzen (v8 überschrieb die ganze Liste) | [20 §2](20-Funktionen.md), [30 §2.3](30-NFR-und-Persistenz.md), [14 §2](14-Dateihandling.md) | `sym:exportEntryTemplates` | offen |
