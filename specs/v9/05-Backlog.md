...
| Ⓙ Bearbeitung & Formulare | BL-233 · **BL-352 → BL-353 → BL-354** (die Erfassungs-Vorlagen in dieser Reihenfolge; ihre Kern-Primitive — Modell, Persistenz, `applyEntryTemplate` — ist gebaut, ab hier reine UI-Arbeit) · BL-334 (Meldungs-Kanäle) | `ui/shell/StatusNotice.svelte`, `ui/views/person`, `ui/views/place`, `ui/views/hof`, `ui/views/media`, `ui/shell` (Formulare, `SourceCitationRow`, Ablage, `EventEditModal`), `core/model/entry-templates.ts`, `services/app-data/`, `tests/ui/touch-target.test.ts` |
...
5. **Welle 3 — Infra & Hygiene (opportunistisch):**
   ...
   **BL-352 · BL-353 · BL-354** (die Erfassungs-Vorlagen, [ADR-v9-264](04-Entscheidungslog.md#adr-v9-264) —
   in dieser Reihenfolge und nicht beliebig teilbar; die Kern-Primitive darunter ist
   gebaut, diese drei sind ihre Konsumenten und ab hier reine UI-Arbeit) · BL-322 (die beiden
