...
| Ⓖ Navigation & Shell | BL-359 (gemessener Überlauf, unabhängig) · BL-94 (Inhalt zuerst spezifizieren) | Shell / Navigation, `ui/shell/UndoControls.svelte`, `swipe-nav.ts`, Modal-Schalen |
...
| Ⓙ Bearbeitung & Formulare | BL-233 | `ui/shell/StatusNotice.svelte`, `ui/views/person`, `ui/views/place`, `ui/views/hof`, `ui/views/media`, `ui/shell` (Formulare, `SourceCitationRow`, Ablage, `EventEditModal`), `tests/ui/touch-target.test.ts` |
...
3. **Welle 2 — eigenständige Kür:** BL-230 (die Ebenen-Wahl hängt sich in das gebaute
   B1-Bündel ein) · BL-207 · BL-233 · BL-359 (gemessener Kopfleisten-Überlauf bei 375 px,
   Nebenbefund aus [ADR-v9-269](04-Entscheidungslog.md#adr-v9-269) — vorbestehend, hängt an
   nichts und blockiert nichts). **Drei Zeilen dieser Welle sind am 2026-08-03 gebaut**
