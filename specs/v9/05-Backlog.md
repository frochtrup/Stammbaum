...
| Ⓞ Zeitachse der Orte | BL-326 (die Sprachachse, unabhängig und noch unentschieden) | `core/places/types.ts`, `core/places/place-registry.ts`, `core/places/gov.ts`, `services/places/types.ts`, `ui/views/place/PlaceEnclosureEditModal.svelte`, `ui/views/place/PlaceNamesSection.svelte` |
...
4. **Welle 6 — die Zeitachse der Orte (2026-08-09, [ADR-v9-243](04-Entscheidungslog.md#adr-v9-243)):**
   BL-326. Die beiden ersten Schritte dieser Welle — der fehlende Überlappungs-Hinweis
   und die Tagesgenauigkeit selbst — sind am 2026-08-09 gebaut und stehen deshalb nicht
   mehr hier. **BL-326 ist bewusst zuletzt und unabhängig:** es ist die einzige der drei
   Zeilen, deren Entscheidung noch aussteht, und sie berührt mit der Match-Namensmenge
   ([11 §4.2](11-Orte-Hoefe-Identitaet.md)) die Ereignis-Auflösung — das ist keine
   Anzeige-Frage. **Herkunft:** Nutzer-Vorgabe „tagegenau abbildbar", gemessen am
   maßgeblichen Bestand (`Testdateien/orte-2.json` rev 277 +
   `Testdateien/Unsere Familie 2026.ged`), nicht aus dem Spec geschlossen.
