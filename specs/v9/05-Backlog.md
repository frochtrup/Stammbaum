...
| Ⓖ Navigation & Shell | BL-320 (Muster steht, sieben Flächen fehlen) · BL-94 (Inhalt zuerst spezifizieren) | Shell / Navigation, `swipe-nav.ts`, Modal-Schalen, die sechs `*List.svelte`, `TasksView`/`LogView`/`HypothesesView` |
...
sieben weitere Flächen.

1. **Welle 5 — was ein Nutzer heute trifft:** BL-320 · BL-318 · BL-315. BL-320 zuerst: die
   Ursache ist bekannt, die betroffenen Flächen sind ausgezählt und das Muster steht schon
   (dieselbe Klasse wie die am 2026-08-08 gebaute Dashboard-/Suche-Zeile, s. Absatz
   darüber) — es ist Anwenden, kein Entwerfen. BL-318 und BL-315 sind Befunde am laufenden
...
|---|---|---|---|---|---|---|---|
| BL-320 | — | defekt | basis | **Dieselbe Klasse wie BL-319 an sieben weiteren Flächen: Suche, Filter und Anzeige-Modus überleben die Navigation nicht** — Geschwister-Population, im Zug von [ADR-v9-229](04-Entscheidungslog.md#adr-v9-229) **gemessen, nicht vermutet** (2026-08-08, `let … = $state` je Datei): `PersonList` (`sortMode`, `query`, `filters`) · `FamilyList` (dieselben drei) · `PlaceList` (`query`, `filters`, `groupMode`, `section`) · `HofList` (`query`, `section`, `filters`) · `TasksView` (`filter`, `viewMode` — der Kanban-/Listen-Umschalter) · `LogView` (`filter`, `logMode` — der Zeitleisten-Umschalter) · `HypothesesView` (`filter`). **Der Weg, auf dem es auffällt:** auf Mobil ersetzt das Detail die Liste (`EntityTab`, `{#if hasSelection}`) — wer aus einer gefilterten Liste eine Person öffnet, kommt in die ungefilterte zurück; bei den drei Forschungs-Segmenten reicht schon der Wechsel des Nav-Ziels. `SourceList`/`RepositoryList` sind NICHT betroffen (sie haben kein Suchfeld), `MediaGallery` ist es seit [ADR-v9-192](04-Entscheidungslog.md#adr-v9-192) nicht mehr. **Zwei Anzeige-Modi sind zusätzlich ein Merker-Fall:** `viewMode`/`logMode` sind strukturgleich zu `mapMode`/`treeMode`/`storyMode`, die als Route-Merker längst überleben — die Regel war an vier Lens-Stellen gebaut und an diesen zwei nicht. **Fertig-Zustand:** je Fläche ein Halter nach dem in ADR-v9-229 festgelegten Muster (die zwei Anzeige-Modi stattdessen als Route-Merker, weil dort ihre Geschwister liegen), Eigentümer je nach Weg (`EntityTab` reicht für Liste→Detail→Liste NICHT: der Sprung in eine Lens baut auch ihn ab), und je Fläche ein Test, der den ganzen Weg geht: setzen → wegnavigieren → zurück → **hinsehen** | [21 §5](21-UI-UX.md), [20 §1.4](20-Funktionen.md), [ADR-v9-229](04-Entscheidungslog.md#adr-v9-229) | `txt:BL-320@tests/ui/PersonList.component.test.ts` | offen |
