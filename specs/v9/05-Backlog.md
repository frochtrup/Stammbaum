...
| Ⓖ Navigation & Shell | BL-399 (Leerzustands-Wortlaut, unabhängig) · BL-359 (gemessener Überlauf, unabhängig) · BL-369 (gemessene Trefferzone, unabhängig) · BL-94 (Inhalt zuerst spezifizieren) | Shell / Navigation, `ui/shell/UndoControls.svelte`, `swipe-nav.ts`, Modal-Schalen, `ui/islands/tree/hourglass-tree.css` |
...
4. **Welle 6 — die Zeitachse der Orte (2026-08-09, [ADR-v9-243](04-Entscheidungslog.md#adr-v9-243)):**
   BL-326 · BL-394 · BL-396. Vier Schritte dieser Welle — der fehlende Überlappungs-Hinweis, die
   ...
   für den konstruierbaren, nicht für den vorgefundenen Fall. **BL-396 ebenso:** die
   Vorbedingung des automatischen Hof-Nachlaufs ist am Bestand leer (0 Dörfer) — die
   Zeile steht für die Divergenz zweier Heuristiken, nicht für einen gemessenen Schaden.
6. **Welle 3 — Infra & Hygiene (opportunistisch):**
   BL-399 (Wortlaut, am billigsten beim naechsten Anfassen des Leerzustands) · BL-330 (stiller Verlust, aber nur auf dem GRAMPS-Weg und nur an zwei Zitat-Listen, die
...
| BL-396 | S | defekt | kür | **Der AUTOMATISCHE Hof-Nachlauf nach einem Dorf-Merge nutzt eine veraltete Gewinner-Heuristik** — `pickHofWinner` (`core/places/commands.ts`) rangiert nach Verwendungszahl → Koordinaten → Notiz → kleinste ID und hat weder `kuratiert` ([ADR-v9-225](04-Entscheidungslog.md#adr-v9-225)) noch die Kette aus [ADR-v9-296](04-Entscheidungslog.md#adr-v9-296) mitbekommen. Er ist die **riskantere** der beiden Stellen: er läuft ohne Nutzer und führt wirklich zusammen, während der Dedup-Dialog nur vorschlägt — genau der Schaden aus ADR-v9-225 (der Gewinner behält seit [ADR-v9-222](04-Entscheidungslog.md#adr-v9-222) nur seine eigenen Angaben) träte hier ohne Klick ein. **Am Bestand heute folgenlos:** 0 Dörfer mit ≥2 gleichadressigen Höfen unter 183 Höfen (`Testdateien/orte-5.json`) — die Vorbedingung ist leer, deshalb `kür` und nicht dringend. **Fertig-Zustand:** eine Heuristik statt zweier (der Kern-Pfad braucht `PlaceContext`-frei berechenbare Merkmale — prüfen, ob `pickWinnerId` dorthin wandern kann, statt sie zu kopieren), plus ein Test, der einen kuratierten Hof gegen einen rohen Bootstrap gleicher Adresse stellt und den Merge-Nachlauf darüber laufen lässt | [11 §9.2](11-Orte-Hoefe-Identitaet.md) | `test:tests/core/hof-nachlauf-gewinner.test.ts` | offen |
| BL-399 | S | hygiene | kür | **Der Leerzustands-Satz passt nicht mehr zum leer begonnenen Baum** — `nav-model.ts::emptyListHint` sagt „Keine Personen geladen — unter ‚Mehr‘ eine Datei öffnen, um zu starten“. Wer über „Leer beginnen“ ([ADR-v9-297](04-Entscheidungslog.md#adr-v9-297)) gestartet ist, hat genau das entschieden und bekommt trotzdem die Aufforderung, eine Datei zu öffnen. Der Satz stammt aus der Zeit, in der „geladen“ und „aus einer Datei“ dasselbe war. **Kein Defekt** — nichts geht verloren, der Weg („＋ Neue Person“) steht direkt daneben. **Fertig-Zustand:** der Hinweis unterscheidet „nichts geladen“ von „geladen, aber leer“ und nennt im zweiten Fall den nächsten Schritt statt des Datei-Öffnens; ein Test hält beide Wortlaute fest | [20 §1.1](20-Funktionen.md) | `test:tests/ui/leerzustand-hinweis.test.ts` | offen |
