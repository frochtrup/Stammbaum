...
| Ⓞ Zeitachse der Orte | BL-331 (die dritte Eingabefläche) · BL-326 (die Wahlregel, unabhängig und noch unentschieden) | `core/places/types.ts`, `core/places/place-registry.ts`, `core/places/gov.ts`, `services/places/types.ts`, `ui/views/place/PlaceEnclosureEditModal.svelte`, `ui/views/place/PlaceNamesSection.svelte`, `ui/views/hof/HofDetail.svelte` |
...
4. **Welle 6 — die Zeitachse der Orte (2026-08-09, [ADR-v9-243](04-Entscheidungslog.md#adr-v9-243)):**
   BL-331 · BL-326. Die beiden ersten Schritte dieser Welle — der fehlende
   Überlappungs-Hinweis und die Tagesgenauigkeit selbst — sind am 2026-08-09 gebaut und
   stehen deshalb nicht mehr hier. **BL-331 zuerst:** es ist die dritte, beim Bau der
   Tagesgenauigkeit übersehene Eingabefläche derselben Regel und verliert heute stumm
   Daten — ein Defekt, keine Erweiterung. **BL-326 bewusst zuletzt und unabhängig:** seine Entscheidung
...
| BL-331 | — | defekt | basis | **Die Hof-Adresszeile nimmt keinen Stichtag — und leert die Periode still** — [ADR-v9-243](04-Entscheidungslog.md#adr-v9-243)/BL-324 hat die Grenz-Eingabe („1400" oder „8 MAY 1945") an `PlaceNamesSection.svelte` und `PlaceEnclosureEditModal.svelte` gebaut, die dritte datierte Liste aber nicht mitgezogen: `HofDetail.svelte` (`updateAddrFrom`/`updateAddrTo`) rechnet weiter mit `Number(raw)`. `withUpdatedHofAddr` nimmt bereits `GrenzEingabe`, der Kern kann es also — belegt: `Number('8 MAY 1945')` ergibt `NaN`, und `alsGrenze` speichert daraus `from: null`, die Periode ist **ohne Meldung weg**. Genau die Falle, gegen die `Lesung`/`grenzeAusEingabe` gebaut wurden ([11 §1](11-Orte-Hoefe-Identitaet.md): „Die Eingabe lehnt ab, statt zu raten"). **Fertig-Zustand:** die Zeile nutzt `grenz-feld.ts` wie ihre beiden Geschwister, Unlesbares springt sichtbar zurück; ein Test über alle drei Flächen | [11 §1](11-Orte-Hoefe-Identitaet.md), [ADR-v9-243](04-Entscheidungslog.md#adr-v9-243), [ADR-v9-245](04-Entscheidungslog.md#adr-v9-245) | `sym:grenzeAusEingabe@ui/views/hof/HofDetail.svelte` | offen |
