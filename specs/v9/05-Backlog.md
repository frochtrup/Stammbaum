...
4. **Welle 6 — die Zeitachse der Orte (2026-08-09, [ADR-v9-243](04-Entscheidungslog.md#adr-v9-243)):**
   ...
   geschlossen.
5. **Welle 2a — die Zeitrechnung der Ortsaufloesung, aufgedeckt am 2026-08-26 ([ADR-v9-286](04-Entscheidungslog.md#adr-v9-286)):**
   BL-385 zuerst und unabhaengig — die fehlende Monats-Sprosse ist ein reiner Rechenfehler
   in `spanneVonEreignis` mit gemessener Folge (die Kette kippt auf eine Periode, die erst
   ein halbes Jahr spaeter beginnt), und ihr Fix ist die dritte Sprosse einer Leiter, die
   der Kopfkommentar von `zeitbezug.ts` bereits beschreibt. **BL-384 danach, weil es eine
   Entscheidung braucht, keinen Fix:** was ein UNDATIERTES Ereignis als `PLAC` bekommen
   soll, ist in [11 §5](11-Orte-Hoefe-Identitaet.md) als „bewusst" festgehalten, aber nie
   begruendet worden; beide Antworten (volle undatierte Kette / gar keine Reprojektion)
   sind vertretbar und aendern Dateiinhalt (LP-1). **Herkunft:** zwei Nutzer-Befunde am
   eigenen Bestand, gemessen an `Testdateien/Unsere Familie 2026-4-2-2-2.ged` +
   `orte-2.json` (rev 448) ueber den echten Ladepfad. Beide sind Geschwister der Identitaets-Achse, die
   [ADR-v9-286](04-Entscheidungslog.md#adr-v9-286) bereits geschlossen hat (Objekt schlaegt Text).
...
|---|---|---|---|---|---|---|---|
| BL-384 | M | defekt | kür | **Ein undatiertes Ereignis verliert bei der Reprojektion die ganze Verwaltungskette** — `buildFormString` liefert bei `when == null` nur den atomaren Blattnamen (`core/places/build-plac.ts`, so seit dem ersten Commit, ohne ADR), und `reprojectEventsOf` schreibt das ungebremst in `ev.place` — anders als `alignCuratedEventTexts`, das als EINZIGE von sieben `ev.place`-Schreibstellen die Verarmungs-Sperre trägt ([ADR-v9-224](04-Entscheidungslog.md#adr-v9-224)). Gemessen an `Testdateien/Unsere Familie 2026-4-2-2-2.ged` + `orte-2.json` (rev 448): 9 Ereignisse, 80 Segmente, u. a. `Nienberge, Kreis Münster, Regierungsbezirk Münster, Provinz Westfalen, Königreich Preußen, Deutscher Bund` → `Nienberge`. **Offen ist die Entscheidung, was ein undatiertes Ereignis als `PLAC` bekommen SOLL** — volle undatierte Kette (behauptet eine politische Ebene, die zur unbekannten Zeit vielleicht nicht galt) vs. gar keine Reprojektion (Quelle bleibt Wahrheit); [11 §5](11-Orte-Hoefe-Identitaet.md) hält die heutige Kappung als „bewusst" fest, ohne Begründung. **Fertig-Zustand:** eine Entscheidung per ADR, die Sperre an den übrigen sechs Schreibstellen, und ein Test je Richtung | [11 §5](11-Orte-Hoefe-Identitaet.md), [13 §2](13-Interop-Roundtrip.md) | `test:tests/core/undatierte-projektion.test.ts` | offen |
| BL-385 | S | defekt | kür | **Ein monatsgenaues Datum wird auf das ganze Jahr verbreitert** — `spanneVonEreignis` (`core/places/zeitbezug.ts`) kennt nur zwei Sprossen (Tag / Jahr): `MAY 1808` liefert dasselbe Intervall wie `1808` **und wie `NOV 1808`**. Folge am Nutzer-Fall `@I566053080@` (BIRT `MAY 1808`): die Kette kippt von `Ochtrup, Salm-Horstmar, Grand-Duché de Berg, Rheinbund` auf `Ochtrup, Arrondissement Coesfeld, Departement Ems, …`, das erst ab dem 14.11.1808 gilt — der Tie-Break „spätester Beginn" entscheidet, weil das verbreiterte Intervall beide Perioden trifft. Gemessen an `Testdateien/Unsere Familie 2026-4-2-2-2.ged` + `orte-2.json` (rev 448): 125 Ereignisse mit Monat ohne Tag, 37 ortsgebunden, 2 heute abweichend. Der Kopfkommentar von `zeitbezug.ts` beschreibt die Intervall-Leiter selbst — die mittlere Sprosse fehlt schlicht; [11 §1](11-Orte-Hoefe-Identitaet.md) sagt „zweistufiger Vergleich", die dritte Sprosse ist also eine bewusste Spec-Änderung. **Fertig-Zustand:** Monats-Intervall in `spanneVonEreignis`/`spanneVonDatiert`, Spec 11 §1 nachgezogen, `roundtrip-verify` grün | [11 §1](11-Orte-Hoefe-Identitaet.md) | `test:tests/core/monats-spanne.test.ts` | offen |
