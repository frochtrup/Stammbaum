...
| Ⓝ Notizen (Anzeige & Erhalt) | BL-381 (die Sektion — der sichtbare Teil) · BL-380 (Write-Back, muss vor jeder Bearbeitung stehen) · BL-382 (Bearbeiten; setzt beide voraus) | `ui/views/person/PersonDetail.svelte`, `ui/views/family/FamilyDetail.svelte`, `ui/views/source/SourceDetail.svelte`, `core/interop/write-back.ts` |
...
3. **Welle 2 — eigenständige Kür:** BL-382 (Notizen bearbeiten — steht am Ende der Kette,
   die offene Frage darin ist das fehlende Familien-Formular) · BL-381 (die Notiz-Sektion; sie ist der Grund, aus dem BL-380
   überhaupt jemanden interessiert — ohne Anzeige gibt es nichts zu bearbeiten und nichts
   zurückzuschreiben) · BL-380 (Notiz-Records kennt der GEDCOM-Write-Back nicht —
   von M3 gefunden, [ADR-v9-282](04-Entscheidungslog.md#adr-v9-282); **latent**, weil keine Fläche
   `db.notes` editiert und der Passthrough den Record byte-gleich hält. Steht hier und nicht früher,
   weil er ohne einen Editor für geteilte Notizen gar nicht Ende-zu-Ende verifizierbar ist —
   dieselbe Erwägung wie bei BL-359/BL-369: gemessen, benannt, blockiert nichts) · BL-230 (die Ebenen-Wahl hängt sich in das gebaute
...
|---|---|---|---|---|---|---|---|
| BL-382 | — | feature | basis | **Notizen bearbeiten — dort, wo es heute keine Fläche gibt.** Setzt [BL-381](#offene-punkte) (Anzeige) und [BL-380](#offene-punkte) (Write-Back) voraus: ohne Anzeige gibt es nichts zu bearbeiten, ohne Write-Back liefe der Edit an einem Notiz-Record ins Leere. Drei Lücken, gemessen an `Testdateien/Unsere Familie 2026-4.ged`: **(a) geteilte Notiz-Records** (219 Stück) — beim Bearbeiten muss sichtbar sein, dass der Edit ANDERE Datensätze trifft (184 Verweise auf 185 Records); die Mitverwender gehören an die Zusammenfassungszeile aus BL-381, nicht in eine Warnung nach dem Speichern. **(b) `extraNotes`** — BL-338 baute das Feld für die zweite Inline-Notiz, es ist bis heute weder anzeig- noch editierbar (0× im Bestand, also latent — aber der Weg über den Import bleibt offen). **(c) `Family.noteText`** (7 Stück, 1.579 Zeichen) — **entschieden (Nutzer): nur ein Notiz-Feld an der Sektion aus BL-381, kein Familien-Formular.** Das ist nicht die kleinere Hälfte einer Konsistenz-Frage, sondern die konsistente Antwort: `FamilyDetail` wird bereits **abschnittsweise** bearbeitet (Eltern über Picker, Ereignisse über den Ereignis-Editor, Zitate über die Zitatzeile) — ein Formular wäre dort das Fremdkörper-Muster, nicht sein Fehlen. Die Notiz-Sektion ist auf allen drei Steckbriefen derselbe Baustein und wird auf allen dreien editierbar; die Familie ist damit kein Sonderfall. **Nicht Teil dieser Zeile:** das Löschen geteilter Notiz-Records und der Umgang mit den 35 Records, die niemand referenziert — Verweis entfernen darf den Record NICHT still mitlöschen, alles Weitere ist eine Datenentscheidung. **Glyphen-Semantik — die Unterscheidung, an der sich das Ganze entscheidet:** `🗑` heißt „der Eintrag verschwindet" ([ADR-v9-263](04-Entscheidungslog.md#adr-v9-263) E4) und gilt deshalb nur für die EIGENE Inline-Notiz; an einer geteilten Notiz ist der destruktive Knopf ein `✕` = **Verweis entfernen** — der Record bleibt (s. o.), und die Glyphe sagt das bereits, statt es einer Rückfrage zu überlassen. Gelöscht/entfernt wird über `ConfirmDialog` (TST-27), nicht über natives `confirm()`. **Fertig-Zustand:** eine geteilte Notiz lässt sich ändern, die Änderung steht nach Speichern und Neuladen in der Datei (`net_delta` unverändert 0) und ist bei allen verweisenden Personen sichtbar; die Mitverwender-Angabe steht VOR dem Edit auf der Fläche; `✕` an einer geteilten Notiz lässt den Record im Bestand | [20 §1.4](20-Funktionen.md), [20 §1.5](20-Funktionen.md), [20 §2](20-Funktionen.md), [21 §7](21-UI-UX.md), [21 §10](21-UI-UX.md), [10 §2](10-Domaenenmodell.md), [ADR-v9-263](04-Entscheidungslog.md#adr-v9-263) | `test:tests/ui/notiz-bearbeiten.test.ts` | offen |
| BL-381 | — | defekt | basis | **Notiz-Sektion je Steckbrief — 53 % des Notiztextes ist auf keiner Lesefläche zu sehen, weitere 19 % nur im Bearbeiten-Formular.** Ausgezählt an `Testdateien/Unsere Familie 2026-4.ged`: von 26.194 Zeichen Notiztext sind **14.013 nirgends zu erreichen** — 219 Notiz-Records (12.434 Zeichen, 184 davon an Personen verwiesen) und `Family.noteText` (7 Stück, 1.579 Zeichen; `FamilyDetail` rendert es nicht, ein Familien-Formular existiert gar nicht). Weitere **4.942 Zeichen stehen nur im Bearbeiten-Formular**, auf keiner Lesefläche: `MediaCitation.note` (49 Stück, 1.184 Zeichen — die Referenzzeile der Medien-Detailansicht zeigt Kontext und Primär-Pille, die Notiz erst hinter `✎`) und `Person.noteText` (s. u.). Dazu zwei Fälle ohne Datenbeleg im Bestand, aber derselben Klasse: `extraNotes` (BL-338 baute das Feld, damit die zweite Inline-Notiz nicht verlorengeht — **null Treffer in der gesamten UI**) und `Association.note`/`Citation.note` (gerendert, aber 0×). `Person.noteText` (31 Stück, 3.758 Zeichen) ist **nur im Bearbeiten-Formular** zu lesen, nicht im Steckbrief — live geprüft an der Demo-Datei. **Eine Sektion für alle Notiz-Quellen eines Datensatzes** (eigene · weitere · verwiesene), EIN Baustein für Person, Familie und Quelle statt drei Einzellösungen (INV-UI-4); geteilte Notizen tragen ein Kennzeichen, weil ein Edit dort andere Datensätze trifft. **Darstellung langer Notizen — Schwelle statt Standardkürzung:** ausgezählt sind die Notizen kurz (Median `Event.note` 10 Zeichen, geteilte Notiz 32, `Person.noteText` 84; **100 % der Ereignisnotizen und 97 % der geteilten passen in ≤ 4 Zeilen**), lang ist eine Handvoll — 11 Records über 200 Zeichen, davon 9 **mit eigenen Zeilenumbrüchen**, längster 783 Zeichen (Hofgeschichte). Deshalb: (a) unter der Schwelle nichts tun; (b) über der Schwelle `<details>` wie `ProofSummaryNote.svelte` — **kein `line-clamp`** (im Projekt nirgends vorhanden; abgeschnittener Text bleibt für Strg+F auffindbar und fürs Auge weg, und die Tastaturbedienung müsste nachgebaut werden, LP-8); (c) die Zusammenfassungszeile trägt Information statt „Mehr" — erste Zeile als Anriss plus Umfang, bei einer geteilten Notiz die Mitverwender; (d) `white-space: pre-wrap` wie bei Ort/Hof/Quelle/Hypothesen, denn die Absatzstruktur ist der eigentliche Lesefluss-Gewinn; (e) die Ereigniszeile bleibt unverändert (max 153 Zeichen) und bekommt nur `pre-wrap`; (f) das Bearbeiten-Feld wächst mit dem Inhalt statt fester `rows="2"` (1.540 Zeichen in zwei Zeilen); (g) **die Bedienelemente sitzen an der Notiz-Zeile selbst** (INV-UI-12) und folgen dem Haus-Vokabular: `.stb-icon-btn`, `✎` zuerst, destruktiv außen ([ADR-v9-263](04-Entscheidungslog.md#adr-v9-263) E4, wie Aufgaben-/Protokoll-/Hypothesen-Zeile). Wo eine Notiz aufklappbar ist, ist die Zusammenfassungszeile zugleich Klick-Ziel des `<details>` — die Symbole dürfen den Aufklapper nicht auslösen und der Aufklapper nicht die Symbole (dieselbe Trennung wie beim Gruppen-Header in `EventsByType`). Die Schwelle steht in ZEILEN, nicht in Zeichen; die Umrechnung (~42 Zeichen/Zeile bei 375 px) ist eine Schätzung und wird im Browser gemessen, bevor sie ins Spec geht ([ADR-v9-91](04-Entscheidungslog.md#adr-v9-91)). **Nicht Teil dieser Zeile:** Bearbeiten geteilter Notizen (braucht [BL-380](#offene-punkte) davor, sonst läuft der Edit ins Leere) und ein Familien-Formular. **Fertig-Zustand:** die drei Steckbriefe zeigen jede Notiz-Quelle ihres Datensatzes; ein Test misst die Schwelle (kurze Notiz ohne Aufklapper, lange mit) und dass die Sektion an allen drei Stellen dieselbe Komponente ist | [20 §1.4](20-Funktionen.md), [20 §1.5](20-Funktionen.md), [20 §1.6](20-Funktionen.md), [21 §10](21-UI-UX.md), [10 §2](10-Domaenenmodell.md), [ADR-v9-282](04-Entscheidungslog.md#adr-v9-282) | `test:tests/ui/notiz-sektion.test.ts` | offen |
| BL-380 | — | defekt | basis | **Ein Edit an einem `0 @N@ NOTE`-Record wird still verschluckt** — `applyDatabaseToRoots` kennt INDI/FAM/SOUR/REPO/OBJE, aber keine Notiz-Records; heute folgenlos (keine Fläche editiert `db.notes`, der Passthrough hält den Record byte-gleich), aber asymmetrisch zum GRAMPS-Write-Back und zu `emitNote`. Gefunden von M3 an `Testdateien/Unsere Familie 2026-4.ged`: 219 von 32.030 mutierten Feldern, alle dieser einen Art. **Fertig-Zustand:** ein Edit am Notiz-Record kommt in der Datei an, `net_delta` bleibt 0, und die Ausnahme in `wire-edit-ankunft.test.ts` (d) fällt weg | [13 §2](13-Interop-Roundtrip.md), [32 TST-31](32-Testframework.md) | `test:tests/roundtrip/note-record-write-back.test.ts` | offen |
