...
| Ⓗ Import / Export / Dedup | BL-318 (Rundlauf-Stabilität am neuen Datenstand) · BL-315 (offener Defekt, wartet auf die eigene Diagnose) · BL-207 · BL-313 (Randfall der Autoritätsregel, unabhängig) · BL-166 (vertagt) | Write-Back, Import-Vergleich, Dedup, Export-Pipe |
...
sieben weitere Flächen — **am selben Tag gebaut** ([ADR-v9-230](04-Entscheidungslog.md#adr-v9-230)), Welle 5 ist damit auf die zwei Mess-Zeilen zusammengeschmolzen.

1. **Welle 5 — was ein Nutzer heute trifft:** BL-318 · BL-315. Beide sind Befunde am laufenden
   Programm und warten auf eine Messung: BL-315 lässt sich erst schließen, wenn der nächste
   Fehlversuch die Stelle benennt — die Diagnose dafür ist gebaut
...
|---|---|---|---|---|---|---|---|
| BL-318 | — | defekt | basis | **Der Kurations-Rundlauf ist am neuen Datenstand nicht mehr stabil** — `tests/roundtrip/kurations-rundlauf-realdaten.test.ts` erwartet, dass der zweite Ladepass nichts nachlegt; seit dem Austausch von `Testdateien/Unsere Familie 2026.ged` (2026-08-07 23:19) entstehen dabei **2 Orte mehr** (354 → 356): „Kreis Münster" und „Amt Wolbeck", beides Verwaltungsebenen aus dem Ortsketten-Seed, keine Höfe. **Nicht von [ADR-v9-228](04-Entscheidungslog.md#adr-v9-228) verursacht** — mit gestashten Änderungen tritt derselbe Fehlschlag auf (Kontrollprobe 2026-08-08). Der GEDCOM-Text bleibt stabil, nur `orte.json` wächst; LP-1 ist also nicht berührt, wohl aber die Zusicherung „der zweite Ladepass legt nichts nach". **Fertig-Zustand:** entweder der Seed erzeugt die beiden Ebenen schon im ersten Pass (dann ist der Test wieder grün), oder die Ursache ist ein echter Zwei-Pass-Unterschied und bekommt einen eigenen ADR | [11 §4](11-Orte-Hoefe-Identitaet.md), [ADR-v9-222](04-Entscheidungslog.md#adr-v9-222) | `txt:BL-318@tests/roundtrip/kurations-rundlauf-realdaten.test.ts` | offen |
| BL-315 | — | defekt | basis | **Der Ortsdatei-Import scheiterte in Safari mit `DataCloneError`, ist seit dem Deploy vom 2026-08-07 nicht mehr auslösbar — und niemand weiß, warum** — Nutzer-Meldung 2026-08-07 (Safari, macOS 26): „Import fehlgeschlagen: The object can not be cloned."; nach dem Deploy von [ADR-v9-226](04-Entscheidungslog.md#adr-v9-226)/[227](04-Entscheidungslog.md#adr-v9-227) läuft derselbe Import in Safari durch. **Die Zeile bleibt trotzdem offen, weil das Verschwinden nicht erklärt ist.** Der `idbPut`-Umbau ist funktional identisch zum Vorzustand (dieselbe Transaktion, dieselbe Reihenfolge, zusätzlich nur ein `try`/`catch`, das im Erfolgsfall nichts tut) — ein `catch` behebt keinen `DataCloneError`, es beschriftet ihn; und `main` lag vor dem Merge nur einen CI-Commit hinter `v9-dev`, der Versionssprung erklärt es also auch nicht. Was gemessen und ausgeschlossen wurde, steht in [ADR-v9-227](04-Entscheidungslog.md#adr-v9-227) (Chromium reproduziert nicht; alle drei Schreibvorgänge klonbar; kein Proxy, `db` ist `$state.raw`; das FS-Handle scheidet aus, weil Safari `showOpenFilePicker` nicht kennt). **Verbliebene Spur:** ein transienter Zustand (IndexedDB-Inhalt eines bestimmten Origins, offene Verbindung, halb angewandtes Schema-Upgrade) statt eines Feldes im Schreibwert — dazu passt, dass die Diagnose seither nie ausgelöst hat. **Fertig-Zustand — bewusst als Wächter, nicht als Fix:** entweder der Fehler tritt erneut auf und die eingebaute Diagnose benennt Speicher, Pfad und Feld (dann wird daraus eine gezielte Zeile), oder er bleibt über mehrere Safari-Sitzungen mit Ortsdatei-Import fort, und die Zeile schließt mit dem Vermerk „nicht reproduzierbar" statt mit einer erfundenen Ursache | [30 §2.1](30-NFR-und-Persistenz.md), [14 §6](14-Dateihandling.md), [ADR-v9-227](04-Entscheidungslog.md#adr-v9-227) | `test:tests/services/places-import-safari.test.ts` | offen |
