...
**`addrExtra` — die nicht erkannten `ADDR`-Kinder ([ADR-v9-228](04-Entscheidungslog.md#adr-v9-228)).** `ADR1`/`ADR2`/`ADR3`/`CITY`/`STAE`/`POST`/`CTRY` sind laut GEDCOM 5.5.1 **Index-Kopien** der `ADDR`/`CONT`-Zeilen, keine eigenständigen Angaben — sie werden deshalb NICHT als Felder modelliert, sondern verbatim als `GedNode[]` am Ereignis gehalten (dieselbe Form wie `MediaCitation.extra` und `Source.dataExtra`). Die Anzeige leitet daraus einen Text ab, wenn `addr` leer ist; ein Nutzer-Edit an der Adresse leert `addrExtra`, statt die Kopien zu raten.
