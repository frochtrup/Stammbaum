...
- **Person:** immer offen: Name(given/surname)+Geschlecht, **Geburt** (Datum+Ort+Quellen). Eigene Pills: Präfix+Suffix (ein Pill), Rufname+Nick (ein Pill), Titel, Religion, RESN, E-Mail, Website — sowie je EIN eigener Pill für Taufe, Tod (+Todesursache), Bestattung (nicht gebündelt, jede Sektion bleibt eigenständig benannt) plus generisch „+ Ereignis hinzufügen" für `events[]`.
- **Familie:** immer offen: Eltern-Zuordnung, **Heirat** (Datum+Ort+Quellen), Kinder ±. Eigener Pill für Verlobung, generisch „+ Ereignis hinzufügen" für `events[]`.
