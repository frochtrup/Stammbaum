...
- **Person:** immer offen: Name(given/surname)+Geschlecht, **Geburt** (Datum+Ort+Quellen). **Zwei graphisch getrennte Pill-Gruppen** (Nachtrag 2026-07-06 — eine gemeinsame Reihe vermischte bisher Identität und Ereignisse ununterscheidbar):
  - **Identitäts-Pills** (bei der Identitäts-Sektion): Präfix+Suffix (ein Pill), Rufname, Titel, Religion, RESN, E-Mail, Website.
  - **Ereignis-Pills** (eigene Reihe bei den Ereignissen): je EIN eigener Pill für Taufe, Tod (+Todesursache), Bestattung (nicht gebündelt, jede Sektion bleibt eigenständig benannt) — UND je ein eigener Pill für die häufigsten generischen Ereignisse **Beruf (OCCU)** und **Wohnort (RESI)** (analog v8 `_PF_PILLS`, das genau diese zwei als eigene Einträge führt, nicht nur den generischen Umweg). Klick fügt sofort ein `OCCU`-/`RESI`-Event zu `events[]` hinzu. Der generische „+ Ereignis hinzufügen"-Typ-Dropdown bleibt für alle selteneren Typen (EDUC/EMIG/IMMI/NATU/EVEN/GRAD/ADOP/MILI/FACT/CENS/PROP).
