...
|---|---|---|---|---|---|---|---|
| OE-1 | K | feature | basis | Programm-Skelett: eigener Einstieg `app-orte/` mit eigener Vite-Konfiguration (`root`, `base` `/stammbaum-v9/orte/`, `outDir ../dist/orte`, `emptyOutDir: false`), CSP-Plugin wiederverwendet. Die Konfiguration liegt IM Editor-Baum, damit sein Bau-Wissen mit ihm zusammenbleibt | [22 §2](22-Orte-Editor-Standalone.md) | `datei:app-orte/vite.config.ts` | offen |
| OE-2 | K | feature | basis | Dokument-Lebenszyklus (INV-ORTE-3): öffnen über eigene `PickerAdapter`-Instanz, „Neu", Änderungsmarke, Speichern mit `rev+1`/`device`/`ts` durch `FileService.exportToFile` (Tier 1 in dieselbe Datei, sonst Teilen/Download), Schema-Gate → Nur-Lese-Modus | [22 §4](22-Orte-Editor-Standalone.md) | `sym:createOrteDocument` | offen |
| OE-3 | K | feature | basis | `PlacesHost`-Implementierung des Editors über `makeDatabase()` + geladene Orte/Höfe, mit Undo/Redo über `services/undo` auf Schnappschüssen von `{placeObjects, hofObjects}`; `caps.hasEventContext` folgt der geladenen Kontextdatei, `canEditEvents`/`canNavigateToLens` sind dauerhaft falsch | [22 §3](22-Orte-Editor-Standalone.md) | `sym:createOrteHost` | offen |
| OE-4 | K | feature | basis | Absturz-Wiederherstellung: entprellter Entwurf in IndexedDB, Angebot beim Start, Verfall beim erfolgreichen Speichern. Kein `rev`-Bump, kein Union-Merge, nie Quelle (INV-ORTE-3) | [22 §4](22-Orte-Editor-Standalone.md) | `sym:createOrteDraftStore` | offen |
| OE-5 | K | feature | basis | Orts-Fläche angeschlossen: Liste, Steckbrief, Bearbeiten, GOV-Import, Dubletten/Zusammenführen — geteilte Komponenten unverändert, keine Kopie (INV-ORTE-1) | [22 §3](22-Orte-Editor-Standalone.md) | `test:tests/orte/orte-flaeche.test.ts` | offen |
| OE-6 | K | feature | basis | Hof-Fläche angeschlossen: Liste, Detail, Adressvarianten inkl. Umbenennung, Dubletten/Zusammenführen | [22 §3](22-Orte-Editor-Standalone.md) | `test:tests/orte/hof-flaeche.test.ts` | offen |
...
| OE-8 | K | feature | basis | Dokument-Rundlauf (TST-8-Analogon zu `net_delta=0`): laden → bearbeiten → speichern → erneut laden → identisch, gegen `tools/handbuch/fixtures/orte.json` als Vorlage | [22 §4](22-Orte-Editor-Standalone.md), [32 §1](32-Testframework.md) | `test:tests/orte/dokument-rundlauf.test.ts` | offen |
| OE-9 | S | feature | basis | Ausgeliefertes Editor-Handbuch: der aus Anhang E + Kap. 7/8 erzeugte Extrakt liegt beim Editor und wird von ihm verlinkt (Erzeugung selbst: BL-224) | [22 §8](22-Orte-Editor-Standalone.md) | `datei:app-orte/public/HANDBUCH-ORTE.html` | offen |
| OE-10 | E | feature | kür | Eigener Service Worker / PWA-Installierbarkeit des Editors (Precache + Offline-Fallback wie das Hauptprogramm, eigener Cache-Namensraum) | [22 §2](22-Orte-Editor-Standalone.md), [30 NFR-2](30-NFR-und-Persistenz.md) | `datei:app-orte/sw-register.ts` | offen |
