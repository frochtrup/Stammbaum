...
- (2026-07-19, ADR-v9-105 — die Button-Ursache oben) `EventLine.svelte`s Orts-Link war ein `<button>`. Bei „1930, Rheine, Steinfurt, Nordrhein-Westfalen, Deutschland" @375px lief die Zeile ~7px über den rechten Kartenrand hinaus; bei einer noch längeren Kette rutschte der gesamte Ortsname auf eine eigene Zeile und ließ das Datum allein auf der vorigen zurück (46,5px = 3 Zeilen statt 31px = 2). Umgestellt auf `<span role="button" tabindex="0">` mit Enter/Space-Handler — die bestehenden `getByRole('button', …)`-Tests liefen unverändert weiter, weil Rolle und zugänglicher Name identisch bleiben. Im selben Zug ein zweiter Defekt derselben Zeile: das Trennzeichen stand als literales `, ` am Ende eines `{#if}`-Blocks, dessen nachlaufenden Whitespace der Svelte-Compiler wegtrimmt — gerendert wurde „1930,Rheine". Ein Trennzeichen an einer Blockgrenze gehört als abgeleiteter Wert ins Skript (`{dateSep}`), nicht als Literal ins Markup; ein Mustache mit String-Literal (`{', '}`) ist kein Ausweg, den verbietet `svelte/no-useless-mustaches`. Die Verwaltungszugehörigkeits-Kette in `PlaceDetail` ist vom Button-Problem NICHT betroffen (jedes Kettensegment ist ein eigenes Element, die Umbruchstelle liegt dazwischen).
