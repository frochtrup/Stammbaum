...
**g) Prosa-als-Dokumentation → Label+Disclosure (Punkt 7).** Dauerhaft sichtbare Erklärsätze über Berechnungsmechanismen (`PlaceDetail`s „Aktuell:"/„Verwaltungsgeschichte:", ADR-v9-75) werden zu einem kompakten Label + einer ⓘ-Affordanz (Klick/Tap zeigt die Erklärung in einem Tooltip/Popover, analog Formular-Feldhilfen). Mechanismus-Jargon in Sektionstiteln (z. B. „(Herkunfts-Pillen)") entfällt aus dem sichtbaren Titel, bleibt höchstens als Code-Kommentar.
