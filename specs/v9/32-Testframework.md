...
- **TST-29 — Ein Wächter über zwei Listen prüft BEIDE Richtungen; die teurere ist die, in der die Liste hinterherhinkt.** `coverage-spec.test.ts` fragte „ist jeder als modelliert geführte Tag im Spec-Universum verortet?" — das fängt Tippfehler und verkappte Erweiterungen. Es kann nicht fangen, dass die Liste selbst veraltet: ein Tag, den der Parser projiziert, der aber im Nenner fehlt, sieht für den Report wie ein bewusst nicht modellierter aus. [ADR-v9-249](04-Entscheidungslog.md#adr-v9-249) nahm zehn Ereignistags auf, `MODELED_GEDCOM_TAGS` blieb unverändert, und der Report meldete zwei Monate lang **76/135 statt 87/135** (mit `DATA` waren es elf Positionen, [ADR-v9-267](04-Entscheidungslog.md#adr-v9-267)) — eine Coverage-Zahl, die Bauarbeit unterschlägt, verstellt gerade die Priorisierung, für die sie da ist. Zum Vergleich: `event-tag-drift.test.ts` prüft **fünf** Richtungen zwischen fünf Listen und hat genau deshalb gehalten — die fünfte (`1b`, „hat jeder Erzeuger ein deutsches Label?") kam mit [ADR-v9-305](04-Entscheidungslog.md#adr-v9-305) dazu und belegt denselben Satz noch einmal: auch ein Wächter über vier Richtungen hatte eine offene Umkehrung, und in ihr saß ein Tag. **Wo Ableitung nicht möglich ist, ist Containment der Ersatz:** die Coverage-Liste lässt sich nicht aus den Erkennungsmengen erzeugen (sie ist deren Obermenge — auch Kinder wie `GIVN`/`LATI`/`PAGE` sind modelliert), und ein blinder String-Literal-Match über den Projektions-Code liefert Falsch-Positive (der Grund, aus dem `GRAMPS_MODELED` kuratiert ist). Geprüft wird deshalb die eindeutige Richtung: was der Code **beansprucht** (`ERKANNTE_TAGS` ∪ `EVENT_TAGS` ∪ `SPECIAL_EVENT_TAGS`), muss geführt sein. Damit bewegt sich die Zahl mit dem Bau statt mit der Erinnerung. Verwandt: TST-22 (misst, ob ein Test rot wird), TST-28 (Zusicherung fehlte, Fahrzeug war da).

- **TST-30 — Ein Zensus, der auf seine Form wartet, findet nur, was der Bestand schon trägt; die Form wird deshalb ERZEUGT.** Alle vorhandenen Verlust-Zensen (TST-28 eingeschlossen) fragen: „fehlt nach dem Neubau Text, **der in der Datei steht**?" Sie sind damit an den Bestand gebunden — und ein Wert, den erst die App zur Laufzeit erzeugt, steht dort nie. Genau daran lief [BL-378](05-Backlog.md) vorbei: die Import-Vergleichs-Fläche schreibt Suchbegriffe (`_QUERY`), die die 255-Byte-Grenze reißen; im gepinnten Realbestand (`Testdateien/Unsere Familie 2026-4.ged`, ausgezählt) misst der längste `_QUERY` **119 Zeichen** bei 32 Vorkommen — keine Fortsetzung, also für jeden bestandsgebundenen Wächter nichts zu sehen. Der Verlust brauchte nicht einmal eine Datei: die Arbeitskopie in IndexedDB IST serialisierter GEDCOM-Text ([14](14-Dateihandling.md)), der Zyklus lief vollständig in der App. **Der erzeugte Zensus dreht die Frage um** (`tests/roundtrip/wire-loss-lange-werte.test.ts`): er hängt an JEDEN Textwert des geparsten Modells einen 300-Zeichen-Füller, schreibt zurück, liest neu und vergleicht pfadweise. Damit ist die KLASSE abgedeckt statt des Einzelfalls — ein neues Textfeld im Modell wird automatisch mitgeprüft, ohne dass jemand daran denkt. Er fand auf Anhieb neun weitere Felder derselben Bauart (`PLAC`, `TYPE`, `NAME`, `DATE`, `_INFM`, `EXID`, `REPO`, `DATA.EVEN`, `FILE`), von denen [ADR-v9-266](04-Entscheidungslog.md#adr-v9-266) vier bewusst liegen gelassen hatte, weil sie „heute ohne Daten im Bestand" seien. **Was er nicht prüfen kann, prüft er gezielt:** Werte, die zugleich IDENTITÄT sind (`Media.file` schlüsselt `db.media`), lassen sich nicht mitverlängern, ohne den Vergleichspfad zu verlieren — sie bekommen einen eigenen Test statt einer stillen Ausnahme. Jede Ausnahme in der Ausschlussliste trägt ihren Grund; ein „sicherheitshalber" ausgenommenes Feld ist ein ungeprüftes Feld. Wirkung negativ verifiziert ([ADR-v9-281](04-Entscheidungslog.md#adr-v9-281)).

- **TST-31 — Mutation, Korpus und Zusicherung bleiben getrennt; der Bestand ist Finder, nicht Prüfstein.** Eine handgeschriebene Fixture legt STRUKTUR, INHALT und ZUSICHERUNG zugleich fest — deshalb braucht jedes neue Phänomen eine neue Datei, und ob es sie gibt, hängt an der Erinnerung. Getrennt genügt ein Korpus für viele Fragen: `Korpus → parse → MUTATION → Write-Back → serialisieren → parse → EIGENSCHAFT` (`tests/roundtrip/wire-mutation-harness.ts`). Eine Mutation ist eine Funktion, eine Eigenschaft ein Test. **Gebaut: M1** (jeden Wert über die Byte-Grenze verlängern → kein Text geht verloren, [ADR-v9-281](04-Entscheidungslog.md#adr-v9-281)) und **M3** (jeden Wert ersetzen → der Edit kommt an, UND der alte Wert bleibt nirgends stehen, [ADR-v9-282](04-Entscheidungslog.md#adr-v9-282)). Vier Regeln halten den Raum klein und ehrlich:
  1. **Kein Kreuzprodukt.** Nicht jede Eigenschaft passt zu jeder Mutation — „kein Text verschwindet" ist für eine Lösch-Mutation sinnlos. Je Mutation wird die Handvoll Eigenschaften gebaut, die für sie eine Frage ist; die Lücken sind benannt, nicht vergessen.
  2. **Deterministisch erschöpfend statt zufällig.** Die Wanderung trifft jedes beanspruchte Feld genau einmal: keine Seeds, kein Shrinking, kein flackerndes CI, und der Fehlschlag nennt den Pfad. `fast-check` bleibt den Fragen vorbehalten, deren Gegenstand eine KOMBINATION ist (Folgen von Edits, Merge-Reihenfolgen).
  3. **Die Ausschlussliste ist der Bericht.** Sie sagt, was das Modell NICHT als Text beansprucht (Zeiger, normalisierte Aufzählungen, aus dem verbatim geschriebenen HEAD Abgeleitetes, beim Lesen Eingefrorenes) — eine Auskunft, die sonst nirgends steht. Sie wohnt in der Naht, nicht in der einzelnen Mutation: zwei Kopien wären zwei Meinungen über das Modell. Jede Zeile trägt ihren Grund; ein „sicherheitshalber" ausgenommenes Feld ist ein ungeprüftes Feld.
  4. **Die erste Meldung eines neuen Wächters wird geprüft, nicht geglaubt.** M3 meldete ein „verlorenes" `WWW`, das in der Ausgabe nachweislich dastand: die Mutation hatte einen GEDCOM-Tag im Passthrough-Baum mitverfälscht, worauf der Lexer die Zeile verwarf — **wer ein Modell generisch mutiert, mutiert auch dessen Struktur.** Vier weitere Treffer waren Substring-Zufälle in legitimem Passthrough. Und umgekehrt gilt: ein Finder, dessen Rot-Fall nie gesehen wurde, ist unbelegt — M3 führt seinen an einer gepflanzten Spiegelung vor.
  Damit ist auch das „Testen an einer Datei"-Problem (TST-21, [ADR-v9-178](04-Entscheidungslog.md#adr-v9-178)) strukturell erledigt: die Zusicherung hängt am eingecheckten Korpus, der Bestand beantwortet nur noch, WELCHE Kombinationen wirklich vorkommen. Bisher machte die falsche Datei die Zahlen falsch; jetzt macht sie nur die Entdeckung schwächer. Geprüft wird dort die MENGE der nicht ankommenden Pfad-Arten — heute einelementig und benannt ([BL-380](05-Backlog.md)); eine neue Art macht rot und nennt sich selbst.
...
| [14](14-Dateihandling.md) | INV-FILE-1 (eine Arbeitskopie), INV-FILE-2 (ein Export-Rohr, alle Formate), INV-FILE-3 (Tier-Auswahl einzige Verzweigung), INV-FILE-4 (nie ohne benannte Sicherung überschreiben) | Unit (Adapter gemockt) |
