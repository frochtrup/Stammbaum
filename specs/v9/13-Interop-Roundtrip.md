...
- **Jeder Zeilenwert wird MIT seinen Fortsetzungen gelesen** (`collectText`) — ohne Ausnahme, auch der Wert eines Ereignisses (`1 EVEN …`/`1 OCCU …`). Ein roh gelesener Wert endet nach dem ersten Fragment, und weil `CONC`/`CONT` bewusst nicht als un-modellierte Kinder durchreisen (s. Passthrough-Ausnahme oben), schreibt der Neubau des Records die Kürzung in die Datei: der Rest ist dann weg. Umgekehrt gilt: sobald ein Wert ein `\n` tragen kann, bildet der Emitter ihn über den Textknoten ab (Wert + `CONT`-Kinder), und die Fortsetzungen stehen **vor** allen anderen Kindern des Knotens.
- **Die Regel wohnt in den zwei geteilten Helfern, nicht in je einem Feld** ([ADR-v9-281](04-Entscheidungslog.md#adr-v9-281)). Lesend faltet `childValue` selbst die Fortsetzungen — wer einen Wert über den Tag-Namen erfragt, bekommt ihn ganz, ohne daran zu denken. Schreibend zerlegt der Serialisierer einen Wert mit `\n` in `CONT`-Zeilen, bevor die Byte-Grenze greift; der Textknoten des Emitters bleibt daneben, als Baumform für den Passthrough-Vergleich. Der Grund ist die Reichweite: die Grenze trifft **jeden** Tag, nicht nur die, für die das Format Fortsetzungen vorsieht — unser eigener Serialisierer bricht jeden Wert über 255 Bytes um, auch einen `_QUERY`, den erst die App erzeugt hat.
