...
„0 Vorkommen" im Zensus heißt damit belegbar „nicht in diesen Dateien", nicht „nicht in der Spec". Die GRAMPS-Modell-Seite ist KONSERVATIV kuratiert (unsichere Elemente zählen als passthrough → die At-Risk-Menge ist eine sichere Obergrenze, kein verstecktes Risiko). Offen bleibt eine vollständige GEDCOM-**7.0**-Enumeration (die maschinenlesbare FamilySearch-Registry statt des unvollständigen HTML-Summarizers) — die 7.0-Zusätze, die das Modell heute handhabt, sind code-gegründet als `GEDCOM_7_ADDITIONS` geführt.
