...
|---|---|---|
| GEDCOM | `INDI/FAM>_STAT` (44/2), `ADDR>CITY/POST/ADR1/_MAP/_LATI/_LONG` (`event.addr` ist Flach-String), `NOTE>_VALID`, `OBJE>_SCBK` (via `MediaCitation.extra`, edit-sicher) | `REFN`→`exids`, `_UID`→`uid`, Namensteile, Ereignisse, Zitate, Medien, `SOUR>DATA/AGNC` + `_DATE` (BL-217/243) |
