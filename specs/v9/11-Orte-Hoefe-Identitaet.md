...
- **`from`/`to` werden aus `fromDate`/`toDate` ABGELEITET** — das Jahr ist nie eine zweite, unabhängige Angabe, sondern die gröbere Fassung des Tages. Wer die Tagesangabe setzt, setzt das Jahr mit. Die Richtung ist Teil der Regel: der Tag zieht das Jahr, nie umgekehrt, und nur ein **tagegenauer** Stichtag zieht (`ABT 1700` trägt ein Jahr, aber keine Tagesgenauigkeit — es bleibt, wie es steht). Durchgesetzt wird sie an den zwei Türen, durch die fremde Bytes zu Orten werden ([ADR-v9-248](04-Entscheidungslog.md#adr-v9-248)): der gewählten Datei (`parsePlacesFileWrapper`) und dem IDB-Spiegel (`PlacesSyncService.ladeSpiegel`, auch für den Remote-Vergleich beim Speichern — sonst vergliche der Union-Merge eine abgeleitete gegen eine rohe Seite). Im Programm entstandene Grenzen sind schon vorher kongruent: die vier `with…`-Kommandos und die GOV-Anreicherung setzen beide Hälften aus einer Quelle. `tests/core/grenzen-kongruenz.test.ts` bewacht weiterhin genau das, was die Ableitung nicht tut — die beiden Schreibpfade, und den Realbestand **roh** gelesen, damit eine Drift in der Kuration auffällt, bevor der nächste Speichervorgang sie heilt.
- **Die Eingabe lehnt ab, statt zu raten.** Ein Grenz-Feld nimmt eine Jahreszahl („1810"), einen Stichtag in kanonischer („1 OCT 1810") oder deutscher Form („1. Oktober 1810", „1.10.1810") und einen leeren Wert (= offen). Alles andere wird **abgelehnt**, und das Feld springt sichtbar auf den gespeicherten Wert zurück; eine Neuanlage nennt den Grund. Die Monatsnamen kommen aus `normalizeMonth` — derselben Funktion wie im Ereignis-Formular, nicht aus einer zweiten Auslegung (INV-UI-4). **Grund:** ein Feld, das Unlesbares als „offen" liest, leert die Periode still, und eine geleerte Zuordnung springt in der Sortierung nach vorn — die nächste Korrektur trifft dann eine andere Zeile.
- **Der Zeitbezug wird durchgereicht, nicht auf das Jahr verkürzt ([ADR-v9-246](04-Entscheidungslog.md#adr-v9-246)).** Jeder Pfad, der gegen datierte Perioden vergleicht — Auflösung, Sofort-Reprojektion, Textangleich — übergibt `eventSpanne(ev)`, nicht `eventYear(ev)`. Ein Jahr an dieser Naht holt für zwei Perioden desselben Jahres den Tie-Break zurück, den die Kuration gerade aufgelöst hat, und lässt die tagegenaue Kette wirkungslos. `eventYear` bleibt für alles, was wirklich ein Jahr will (Lebensalter, Zeitleisten-Achse, Statistik).
- **Der Vergleich ist GESTUFT, nie tagesgenau erzwungen:** tagegenau nur, wenn Ereignis- *und* Ortsseite einen Tag tragen; tragen sie einen Monat, gilt der Monat; sonst Jahr gegen Jahr. Die drei Sprossen sind keine Fallunterscheidung, sondern die BREITE eines Intervalls ([ADR-v9-243](04-Entscheidungslog.md#adr-v9-243), Monats-Sprosse nachgerüstet mit [ADR-v9-287](04-Entscheidungslog.md#adr-v9-287) — `MAY 1808` war bis dahin identisch zu `NOV 1808`). Ein QUALIFIZIERTES Datum (`ABT`/`BEF`/`AFT`/…) fällt immer auf sein Jahr zurück, auf jeder Sprosse. Am Realbestand (`Testdateien/Unsere Familie 2026.ged`) sind 6604 von 9377 Ereignisdaten tagegenau, 2822 nicht, und 747 tragen einen Qualifier (`ABT`/`BEF`/`AFT`/`BET`/`FROM`/`CAL`), bei dem ein Tag Scheingenauigkeit wäre. Ein Modell ohne Jahres-Rückfall wäre für ein Drittel des Bestands schlechter, nicht besser.
...
**Überlappende `enclosedBy`-Perioden — Tie-Break-Regel:** widersprechen sich zwei `enclosedBy`-Einträge für dasselbe Jahr (z. B. fehlerhaft importierte oder manuell überlappend eingetragene Zeiträume), gewinnt der Eintrag mit dem **höheren Startjahr** (`from`) — die zeitlich näher liegende Periode. `enclosureIdsAsOf`/`enclosureChainAsOf` melden den Fall über `EnclosureMeta.ueberlappt`, der Ort-Steckbrief zeigt ihn als ⚠ an der betroffenen Jahres-Zeile; still gewählt wird trotzdem deterministisch, kein Blockieren der Projektion.

Der Hinweis gilt für die **ganze Kette**, nicht nur für den Blattknoten: eine Mehrdeutigkeit auf einer übergeordneten Ebene bestimmt den Rest der Kette genauso. Eine **undatierte** Zugehörigkeit zählt dabei **nicht** als Konkurrenz — sie ist der Rückfall („ohne bekannte Datierung"), kein zweiter Anspruch; sonst trüge nach jedem Merge (der mehrere undatierte Ketten hinterlässt, [ADR-v9-72](04-Entscheidungslog.md#adr-v9-72)) jedes Jahr das Zeichen. `buildPlacForGedcom` markiert **nicht**: es baut den Wire-Wert für die Datei, und ein Warnhinweis hat dort keinen Adressaten (frühere Fassung dieses Absatzes behauptete es, ohne dass es je gebaut war — BL-325).
...
**Für `pnames` gilt derselbe zweistufige Vergleich — die Konflikte dort sind aber von zweierlei Natur ([ADR-v9-245](04-Entscheidungslog.md#adr-v9-245)).** Dieselbe Messung zählt 199 Randberührungen, davon **102 mit einem Verfassungs-/Statuswort** („Grafschaft Oldenburg [1100–1774]" × „Herzogtum Oldenburg [1774–1806]", „Königreich Bayern [1806–1918]" × „Freistaat Bayern [1918…]"). Sie tragen einen rechtlich datierbaren Stichtag und werden von ihm genauso eindeutig wie die Zugehörigkeiten. **Kurationsform: `toDate` des Vorgängers ist der Tag VOR dem Stichtag** — die Spannengrenzen sind inklusiv, zwei tagegenaue Nachbarperioden mit demselben Grenztag überlappen sonst am Stichtag selbst noch.

**Daneben stehen 74 echte Überlappungen über mehrere Jahre, und die löst kein Tagesdatum.** Die Namensachse ist — anders als die Hierarchie — **nicht exklusiv**: ein Ort kann gleichzeitig mehrere gültige Namen tragen. Drei Formen kommen vor: identische Perioden („Hochstift Osnabrück" × „Fürstbistum Osnabrück", „Département de la Roer" × „Roer-Departement"), Enthaltensein („Sprockhövel [1300…]" ⊇ „Stadt Sprockhövel [1970…]") und unscharfe Schreibvarianten („Bochem"/„Buchem"/„Bokheim"/„Boeckum"). Heute entscheidet dort dieselbe Vorrangregel (höchstes `from`), aber **ohne Meldung**: `resolveAsOf` kennt kein Gegenstück zu `EnclosureMeta.ueberlappt`. Welcher dieser Namen in `PLAC` gehört, ist eine eigene Frage ([BL-326](05-Backlog.md)) und wird hier nicht mitbeantwortet. **Das ⚠ der Hierarchie-Achse wird dabei ausdrücklich NICHT gespiegelt:** dort ist eine Überlappung ein Fehler, hier ein legitimer Zustand.

**Ohne Stichtag baut die Projektion keine Kette — und ersetzt auch keine ([ADR-v9-292](04-Entscheidungslog.md#adr-v9-292)).** Für ein **undatiertes** Ereignis liefert `buildFormString` nur den atomaren Leitnamen: eine Kette ohne Datum wäre eine erfundene Epoche, die `enclosedBy`-Wahl fiele auf den ersten Eintrag. Der Leitname ist damit die richtige **Konstruktion**, aber kein zulässiger **Ersatz** für eine reichere Quelle: nennt der `PLAC`-Text eine Ortsebene, die diese Projektion nicht trägt, liefert `buildPlacForGedcom` **`null`** — jede Schreibstelle fällt dadurch von selbst auf den Quelltext zurück (dieselbe Bewegung wie beim fehlenden `hofObject`). Geprüft wird das über `verloreneEbenen(ev, ctx, projektion)`, den **Knoten**-Zwilling von `unbekannteEbenen` aus derselben Kettenabdeckung: dort „nennt die Quelle eine Ebene, die der Bestand nicht kennt", hier „läßt die Projektion eine bekannte Ebene weg". Eine periodengerechte **Umbenennung** desselben Knotens ist kein Verlust. Für **datierte** Ereignisse gilt die Sperre nicht — dort hat die Frage seit [ADR-v9-224](04-Entscheidungslog.md#adr-v9-224) ihre eigene, engere Antwort im Textangleich.
...
**`conflict`-Flag und UI-Pflicht (ADR-v9-50).** Eine Gruppe trägt `conflict: true`, wenn mindestens ein Mitglieder-Paar unverträgliche Elternketten hat (§4.2, ADR-v9-29-Regel: eine Kette ist Präfix der anderen, oder leer) — Namensgleichheit allein hat die Gruppe gebildet, nicht wechselseitige Verträglichkeit ALLER Mitglieder. Die UI ([20 §1.7](20-Funktionen.md)) MUSS in diesem Fall die **volle, periodenunabhängige Namenskette** je Mitglied zeigen (`buildFullPlaceName(reg, id)`, §5 — NICHT den bloßen `title`, sonst sind gleichnamige, unterschiedlich verortete Einträge nicht unterscheidbar) plus einen sichtbaren Warnhinweis. Kein zusätzlicher automatischer Gewinner-Bias — die Heuristik bleibt Vorschlag, nie bindend. **Reihenfolge seit [ADR-v9-296](04-Entscheidungslog.md#adr-v9-296): kuratiert → Anreicherungs-Grad → datierte Perioden → Verwendungszahl → kleinste ID.** Zwei Gründe für genau diese Ordnung. **Kuratiert steht vorn, weil die Ereignisse dem Gewinner ohnehin folgen** (`placeRemap`): die Verwendungszahl ist kein Argument FÜR ein Objekt, Kuration schon ([ADR-v9-225](04-Entscheidungslog.md#adr-v9-225)). **Und die Verwendungszahl steht hinten, weil derselbe Satz für sie weiter gilt** — sie misst Relevanz für diesen Stammbaum, nicht die Güte des gepflegten Objekts; die beiden Sprossen davor sind direkte Evidenz am Objekt. Die früheren Einzelfragen `hasCoords`/`hasNote` sind entfallen: sie sind zwei der sieben Facetten, die `placeEnrichmentLevel` (§9.1) bereits zählt.

**Die datierten Perioden sind die Sprosse FÜR DIE EBENEN ÜBER DEM DORF.** Eine Verwaltungseinheit bindet fast nie ein Ereignis direkt — das hängt am Dorf —, also ist die Verwendungszahl dort für alle Mitglieder 0, und kuratierte Knoten tragen ohnehin beide Koordinaten und Notiz. Am Realbestand entschied in 2 von 6 Gruppen der **ID-String**. Gezählt wird die MENGE datierter Einträge über beide Zeitachsen (`enclosedBy` + `pnames`; beim Hof die `addrs`) — **nicht ihr Anteil**: der ist bei 10 von 12 Mitgliedern 1,00, bei einem Land ohne Elter 0/0, und er bestraft den Reicheren (fünf von sechs datierten Kanten verlören gegen eine von einer).

**Die REIHENFOLGE der Gruppen ist die zweite, unabhängige Frage** ([ADR-v9-296](04-Entscheidungslog.md#adr-v9-296)): sortiert wird nach **transitiver Reichweite** — wie viele Ereignisse hängen unter dem reichweitenstärksten Mitglied, direkt oder über die periodengerechte Kette (beim Hof: seine direkte Verwendung, er hat keine Kinder). Schwerste Zusammenführung zuerst, bei Gleichstand der stabile Schlüssel. Die Zahl steht sichtbar an der Gruppe, sonst wäre die Ordnung eine unerklärte Umstellung. **Und je MITGLIED steht die Zahl der datierten Perioden** — die Sprosse, die bei Verwaltungseinheiten den Ausschlag gibt, muss ablesbar sein, sonst ist der Vorschlag ein Orakel. Auch die Null wird ausgeschrieben („keine datierte Periode"): in einer VERGLEICHS-Fläche ist die Abwesenheit die Information — dieselbe Lesart, aus der dort auch die Anreicherungs-Pille bei jedem Mitglied steht (§9.1), anders als in der Liste, wo Abwesenheit der Regelfall und damit Rauschen ist. **Reichweite ist ausdrücklich KEIN Gewinner-Kriterium** — das wäre die Verwendungszahl eine Ebene höher und damit derselbe Fehlschluss, den ADR-v9-225 aufgelöst hat. Vorher schlug der Dialog in der gemessenen Aligse-Gruppe die Seed-Dublette vor — und seit ADR-v9-222 hätte dieser Klick die Kuration gelöscht. `conflict` ist reine Zusatz-Information, kein Gate — Zusammenführen bleibt in JEDEM Fall (ob `conflict` oder nicht) ein expliziter Nutzer-Klick, nie automatisch.
...
**Automatischer Hof-Nachlauf nach Dorf-Merge ([ADR-v9-45](04-Entscheidungslog.md#adr-v9-45), [ADR-v9-48](04-Entscheidungslog.md#adr-v9-48)).** Nach einem Dorf-Merge konsolidiert die App gleichadressige Höfe im Zieldorf automatisch nach — v8s `_reconcileFarmsUnderVillage` hat hier ein Analogon. **Warum automatisch und nicht als zweiter, manueller Dedup-Lauf:** (1) **Entdeckbarkeit** — niemand stößt Massen-Dedup von selbst direkt nach einem thematisch anderen Vorgang noch einmal an. (2) **Es wäre eine Resolver-Regression, keine Unordnung** — `hof-registry.ts::findByAddr` liefert bei ≥2 Kandidaten `null` („strikt eindeutig — sonst Review"), das Ereignis fällt in Review-Klasse C (§6). Zwei gleichnamige Höfe unter verschiedenen Dörfern sind eindeutig auflösbar; nach dem Merge hängen beide am selben Dorf, und bei GEDCOM-Quellen kippt das bei JEDEM folgenden vollen Reload (`hofId` nie persistiert, §2) von „eindeutig" auf „mehrdeutig".
