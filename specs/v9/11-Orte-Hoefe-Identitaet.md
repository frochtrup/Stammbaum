...
6. **Gleichnamige Orte mit widersprüchlicher, aber real identischer Verwaltungshistorie — ENTSCHIEDEN (ADR-v9-50 ✅, ersetzt ADR-v9-49 ♻️).** Zunächst (ADR-v9-49) bewusst als Restklasse OHNE Kern-Fix dokumentiert; nach Nutzer-Rückfrage durch **Kriterium 4** in `findPlaceDuplicates` gelöst (§9.2): gleicher Name + widersprüchliche, aber NICHT völlig fremde Elternketten (gemeinsamer Vorfahre irgendwo, z. B. dieselbe Region trotz Gebiets-/Kreisreform) werden jetzt als `conflict: true`-markierte Gruppe vorgeschlagen, mit voller Namenskette (`buildFullPlaceName`) statt bloßem Titel. Völlig fremde Ketten (kein gemeinsamer Vorfahre, z. B. Oldenburg/Niedersachsen vs. Oldenburg/USA) bleiben ausgeschlossen — ADR-v9-29 bleibt unangetastet.
...
**Eine** Funktion, nicht zwei getrennte — `findPlaceDuplicates(items, kind, toleranceKm = 1)`, reine, deterministische Funktion (analog v8: `items` ist `placeObjects` für `kind ∈ {'places','all'}`, `hofObjects` für `kind='farms'`). Liefert Kandidatengruppen (`{ ids: (PlaceId|HofId)[], conflict?: boolean }[]`, ≥ 2 Mitglieder) nach vier Kriterien (Union-Find):
...
4. **Gemeinsamer-Vorfahre-Brücke (NEU, ADR-v9-50, NUR `kind ∈ {'places','all'}`):** gleicher Name, Elternketten WIDERSPRÜCHLICH (Kriterium 1 hat übersprungen), aber beide Ketten teilen irgendwo mindestens einen gemeinsamen Vorfahren (z. B. dieselbe Region trotz abweichender unmittelbarer Gemeinde-Zugehörigkeit — Gebiets-/Kreisreform, uneinheitliche Quellenkonvention, real derselbe Ort — Spec 11 §8 Restklasse 6 „Arpke"). Die resultierende Gruppe trägt `conflict: true`. Völlig FREMDE Ketten (kein gemeinsamer Vorfahre auf irgendeiner Ebene, z. B. verschiedene Länder — Oldenburg/Niedersachsen vs. Oldenburg/USA) bleiben ausgeschlossen, das schützt den ADR-v9-29-Zweck unverändert.

`kind` unterscheidet `'places'` (Farm/Building ausgeschlossen), `'farms'` (nur Hof-Äquivalent), `'all'` (Migrations-/Test-Zweck).

**`conflict`-Flag und UI-Pflicht (ADR-v9-50):** eine Gruppe trägt `conflict: true`, wenn mindestens ein Mitglieder-Paar unverträgliche Elternketten hat (die Gruppe kam nur über Kriterium 2 oder 4 zustande, nicht über die strikte Kriterium-1-Verträglichkeit). Die UI ([20 §1.7](20-Funktionen.md)) MUSS in diesem Fall die **volle, periodenunabhängige Namenskette** je Mitglied zeigen (`buildFullPlaceName(reg, id)`, §5 — NICHT den bloßen `title`, sonst sind gleichnamige, unterschiedlich verortete Einträge nicht unterscheidbar) plus einen sichtbaren Warnhinweis. Kein zusätzlicher automatischer Gewinner-Bias — die bestehende Heuristik (Verwendungszahl → Koordinaten → Notiz → kleinste ID) bleibt unverändert Vorschlag, nie bindend.
