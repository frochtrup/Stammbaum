...
## 7. Reihenfolge
**§3.0 Verifikation ✅ → P0a-1 ✅ (Zeitachse Parser/Writer, sw v796) → P0a-2 ✅ (PlaceRegistry, sw v797) → P0b-1 ✅ (collectPlaces↔Entität + Ort-Detail-Historie, sw v798) → P0b-2a ✅ (Dubletten-Erkennung + Merge-Kern, sw v799) → P0b-2b ✅ (Merge-Dialog UI, sw v800) → P0b-3 (extraPlaces→placeObjects) → Review → P2-UI → P3 → P4 → P5.**

**Stand (Implementierung, sw v800, test-unit 198 grün):**
...
- **P0b-2b** `ui-views-place.js` + `index.html` + `styles.css`: Orte-Tab ⇉-Button → `modalPlaceMerge`; `openPlaceMergeModal`/`_renderPlaceMergeList`/`placeMergeGroup`/`_placeUsageCounts`; Radio-Gewinnerwahl (Vorschlag = meiste Event-Verwendungen), CSP-safe (`data-action`). Browser-verifiziert.
