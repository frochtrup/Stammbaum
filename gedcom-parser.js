...
function _parseINDILv1(cur, x, tag, val) {
  x.evIdx = -1;
  if (tag === 'NAME') {
    if (cur._nameParsed) {
      const sn2 = (val||'').match(/\/([^\/]*)\//) || [];
...
function _parseINDILv2(cur, x, tag, val) {
...
function _parseINDILv3(cur, x, tag, val) {
  ...
  _parseSourCitSub(cur, x, tag, val);
...
function _parseINDILv4(cur, x, tag, val) {
...
// Shared: lv3 SOUR-Citation-Sub-Tags (PAGE/QUAY/NOTE/OBJE/_EVAL) — identisch in INDI und FAM
function _parseSourCitSub(cur, x, tag, val) {
  if (x.lv2tag !== 'SOUR' || !x._curCit || tag === 'TIME') return;
...
function _parseINDILine(cur, x, lv, tag, val) {
  if      (lv === 1) _parseINDILv1(cur, x, tag, val);
  else if (lv === 2) _parseINDILv2(cur, x, tag, val);
  else if (lv === 3) _parseINDILv3(cur, x, tag, val);
  else if (lv === 4) _parseINDILv4(cur, x, tag, val);
...
function _parseFAMLv1(cur, x, tag, val) {
...
// Helper: read lv3 OBJE-media sub-tags for a FAM event object's media array
function _parseFamEvMediaLv3(mediaArr, tag, val, x) {
  if (!mediaArr.length) return;
  const _om = mediaArr[mediaArr.length-1];
...
function _parseFAMLv2(cur, x, tag, val) {
  if (x.lv1tag === '_TASK' && x._curTask) {
    if      (tag === '_CAT')   x._curTask.category = val;
    else if (tag === '_DONE')  x._curTask.done = val === '1';
    else if (tag === '_TSTAT') x._curTask.status = val;   // RES-PROJ 3a
...
function _parseFAMLv3(cur, x, tag, val) {
  // RES-HYPO (ADR-023): Evidenz-Seite + mehrzeilige Begründung/Schlussfolgerung
  if (x.lv1tag === '_HYPO' && x._curHypo) {
    if (x.lv2tag === 'SOUR' && tag === 'PAGE' && x._curHypoEvid) x._curHypoEvid.page = val;
    else if (x.lv2tag === '_RATIO' && (tag === 'CONC' || tag === 'CONT')) x._curHypo.rationale  += (tag === 'CONT' ? '\n' : '') + val;
  ...
  if (x.lv1tag==='CHAN' && x.lv2tag==='NOTE' && (tag==='CONC'||tag==='CONT'))
    cur.chanNote += (tag==='CONT' ? '\n' : '') + val;
  _parseSourCitSub(cur, x, tag, val);
  if (x.lv1tag==='MARR' && x.lv2tag==='OBJE')  _parseFamEvMediaLv3(cur.marr.media,  tag, val, x);
  if ((x.lv1tag==='ENGA'||x.lv1tag==='ENG') && x.lv2tag==='OBJE') _parseFamEvMediaLv3(cur.engag.media, tag, val, x);
  if (x.lv1tag==='DIV'  && x.lv2tag==='OBJE')  _parseFamEvMediaLv3(cur.div.media,   tag, val, x);
  if (x.lv1tag==='DIVF' && x.lv2tag==='OBJE')  _parseFamEvMediaLv3(cur.divf.media,  tag, val, x);
...
function _parseFAMLv4(cur, x, tag, val) {
  if (x._smEntry !== null && x.lv3tag === 'OBJE' && x.lv2tag === 'SOUR') {
    if      (tag==='FILE')  { x._smEntry.file=val; x._ptDepth=4; x._ptTarget=x._smEntry._extra; }
    else if (tag==='_SCBK') x._smEntry.scbk=val;
    else if (tag==='_PRIM') x._smEntry.prim=val;
...
function _parseFAMLine(cur, x, lv, tag, val) {
  if      (lv === 1) _parseFAMLv1(cur, x, tag, val);
  else if (lv === 2) _parseFAMLv2(cur, x, tag, val);
  else if (lv === 3) _parseFAMLv3(cur, x, tag, val);
  else if (lv === 4) _parseFAMLv4(cur, x, tag, val);
}
